...
- Unified call-log and support-debug proof: the operations record now links trace, replay, provider events, transcript, provider decisions, tools, handoffs, audit, reviews, ops tasks, integration events, sink delivery attempts, telephony media stream lifecycle evidence, failure replay artifacts, and copyable incident Markdown. Session snapshots now expose linked debug artifacts, `createVoiceSessionObservabilityRoutes(...)` gives each call one JSON/HTML/Markdown observability report with turn waterfalls and support links, and `createVoiceCallDebuggerRoutes(...)` gives one latest-session support/debug surface with JSON, HTML, and incident Markdown. React, Vue, Svelte, Angular, HTML, and HTMX all have package-level call-debugger launch primitives. Readiness, ops recovery, evals, simulation suite, tool contracts, outcome contracts, trace timelines, sessions, campaign attempts, and example proof surfaces now link failures back to operations records or the call debugger.
...
- `@absolutejs/voice@0.0.22-beta.459` exposes `buildVoiceSessionObservabilityReport(...)`, `renderVoiceSessionObservabilityHTML(...)`, `renderVoiceSessionObservabilityMarkdown(...)`, and `createVoiceSessionObservabilityRoutes(...)`.
- Session observability turns trace and operations-record data into one per-call support report with provider recovery, fallbacks, tools, handoffs, guardrails, telephony media counts, linked debug surfaces, copyable incident Markdown, and turn waterfalls.
- `@absolutejs/voice@0.0.22-beta.460` adds client/framework access for session observability: fetch/store/widget/custom-element primitives plus React, Vue, Svelte, and Angular wrappers.
- `@absolutejs/voice@0.0.22-beta.461` adds session-observability reports to proof-pack support bundles so buyer-facing proof can show per-call observability coverage, turn waterfalls, and fallback visibility next to snapshots and call debugger reports.
- `@absolutejs/voice@0.0.22-beta.462` adds session-observability evidence assertions with `evaluateVoiceSessionObservabilityEvidence(...)` and `assertVoiceSessionObservabilityEvidence(...)` plus new evidence types that make session observability checks composable in production-readiness gate flows and automated regression tests.
...
- `@absolutejs/voice@0.0.22-beta.456` adds `buildVoiceRealCallEvidenceRuntimeReadinessCheck(...)`, turning rolling real-call evidence into a deploy gate with minimum stored evidence, session, profile, and failed-profile thresholds plus collection/open-runtime recovery actions.
- `@absolutejs/voice@0.0.22-beta.457` adds `createVoiceRealCallEvidenceRuntimeWorker(...)` and `createVoiceRealCallEvidenceRuntimeWorkerLoop(...)`, giving apps an opt-in auto-collector loop with health state so real traffic can continuously feed rolling evidence instead of requiring manual collection.
- `@absolutejs/voice@0.0.22-beta.458` adds `buildVoiceRealCallEvidenceRuntimeWorkerReadinessCheck(...)`, separating evidence-depth readiness from continuous-collection readiness so production gates and proof packs can show whether real-call evidence is manual, idle, stale, failing, or actively running.
- `@absolutejs/voice@0.0.22-beta.463` extends `createVoiceRealCallEvidenceRuntime(...)` with first-class `browserEvidence`, `phoneEvidence`, and `providerRoleEvidence` inputs plus `buildVoiceRealCallProfileEvidenceFromRuntimeSurface(...)` and `buildVoiceRealCallProfileEvidenceFromRuntimeProviderRoles(...)`, so apps can feed browser, phone, provider-role, trace, and reconnect proof into one durable real-call profile history runtime.
