...
Current status: split cleanly. `@absolutejs/media` now owns generic media frames, frame transform pipelines, ordered processor graphs, branch/filter/processor nodes, processor graph reports, resampling plans, calibration reports, VAD reports, interruption reports, transport lifecycle reports, media-quality reports, and WebRTC stats reports. `@absolutejs/voice` depends on media for primitives and keeps voice-specific pipeline proof routes, browser media readiness assertions, provider/realtime integration, operations records, and proof-pack wiring. This is enough to prove app-level media flow in an AbsoluteJS app; it is not yet Pipecat-depth because real browser collection helpers, telephony serializers, richer graph lifecycle controls, debugger/runners, and broader service integration are still roadmap work.
...
- Browser WebRTC stats readiness gate. Status: `@absolutejs/media@0.0.1-beta.4` exposes `buildMediaWebRTCStatsReport(...)`, `collectMediaWebRTCStats(...)`, and `collectMediaWebRTCStatsReport(...)`; `@absolutejs/voice@0.0.22-beta.320` consumes it as `Browser media transport`, stores live browser reports as `client.browser_media` traces, and exposes JSON/HTML proof routes. The example proof pack passes with `.voice-runtime/proof-pack/2026-04-30T09-03-51.415Z`.
- Voice proof consumption for direct WebRTC collection helpers once `@absolutejs/media` exposes browser/mobile transport collectors.
