...
- Realtime/S2S provider integration: voice now owns the shared `RealtimeAdapter` contract, `voice(...)` orchestration path, realtime channel proof, realtime provider contract matrix, and media proof routes that consume `@absolutejs/media` reports. Vendor protocol glue lives in adapter packages. `@absolutejs/voice-openai` and `@absolutejs/voice-gemini` are published beta adapters and pass example provider-contract proof. `@absolutejs/voice@0.0.22-beta.319` consumes `@absolutejs/media@0.0.1-beta.4` media quality and browser WebRTC stats reports in `/voice/media-pipeline`, `/voice/browser-media`, Markdown/HTML where available, proof-pack assertions, production-readiness deploy gates, and operations-record links for impacted sessions. Real browser `RTCPeerConnection.getStats()` collection now exists in media; the remaining voice gap is production-grade provider/live-demo proof and feeding live browser sessions into the proof artifacts automatically.
...
- Browser WebRTC stats readiness gate. Status: `@absolutejs/media@0.0.1-beta.4` exposes `buildMediaWebRTCStatsReport(...)`, `collectMediaWebRTCStats(...)`, and `collectMediaWebRTCStatsReport(...)`; `@absolutejs/voice@0.0.22-beta.319` consumes it as `Browser media transport`; the example proof pack passes with `.voice-runtime/proof-pack/2026-04-30T08-52-04.871Z`.
