...
Current verified proof: core `@absolutejs/voice@0.0.22-beta.432` and the voice example were verified on May 2, 2026.

Latest full proof-pack output remains `.voice-runtime/proof-pack/2026-04-30T15-34-16.230Z` from April 30, 2026. The May 2 verification covered the new incident-timeline primitive, the operational-status primitive, and the updated Absolute standalone build path; rerun `bun run proof:pack:server` when a fresh end-to-end proof artifact is needed.
...
- The voice example installs `@absolutejs/voice@0.0.22-beta.432`, mounts `/api/voice/operational-status`, `/voice/operational-status`, `/api/voice/incident-timeline`, `/voice/incident-timeline`, and `/voice/incident-timeline.md`, installs `@absolutejs/absolute@0.19.0-beta.816`, and defines `"build": "absolute build"`.
...
Next core-product wedge: executable incident recovery. `buildVoiceIncidentTimelineReport(...)` now answers "what broke, when, who was affected, what recovered, and where do I investigate next?" The next buyer question is "can I safely run the recommended recovery from here?" Hosted platforms sell dashboard buttons. AbsoluteJS Voice should answer with primitive-owned action metadata and app-supplied safe handlers that can re-run proof, retry deliveries, open recovery jobs, refresh readiness, or generate support bundles without becoming an opinionated app kit.
