...
- Realtime/S2S provider integration: OpenAI Realtime now has a first-class adapter primitive and initial `voice(...)` orchestration support; the remaining gap is production-grade browser format negotiation/resampling and deeper live-demo proof.
...
| Direct realtime/duplex providers | Covered | Covered to parity | OpenAI Realtime adapter path exists; cascaded STT/LLM/TTS remains strong. Production browser format negotiation/resampling proof needs more depth. | Improve runtime-channel calibration and live-demo proof. |
...
| SIP/media infrastructure | Intentional gap | Lag by design | LiveKit owns rooms, SIP trunks, RTP/SRTP, DTMF, REFER, dispatch, and media networking. | Maintain clean seams for LiveKit/Pipecat/provider transport interop. |
...
- We intentionally lag hosted platforms where the product would become a dashboard/telco/no-code builder: number provisioning, visual workflow builders, and raw media infrastructure.
...
| SIP/media infrastructure | LiveKit is strongest here. | Twilio/Telnyx/Plivo bridges and OpenAI Realtime adapter exist; LiveKit interop is not a goal yet. | Intentional gap | Do not chase full media infra. Expose clean adapter seams so LiveKit/Pipecat can be transport/provider layers when needed. |
...
- OpenAI Realtime adapter path. Status: core adapter primitive exists and `voice(...)` can now run with `realtime` instead of cascaded `stt`/`tts`.
