...
| Prompt-to-agent builder (Vapi Composer, Bland Norm) | **SHIPPED voice@0.0.22-beta.521** — `generateVoicePathwayFromPrompt({ description, completion, id?, guidance?, maxRepairAttempts? })` drives an LLM to emit pathway JSON, parses (markdown-fence tolerant), validates via `validateVoicePathway`, runs a repair loop feeding errors back to the model. Returns `{ pathway, report, attempts, rawOutputs }`. Output is a validated Pathways-DSL (.514) pathway → drop into `compileVoicePathwayToAssistant`. Slugifies description for the id fallback |
| MCP tool bridge (LiveKit Agents 1.5) | **SHIPPED voice@0.0.22-beta.522** — `createVoiceMCPToolset({ client, namePrefix?, allowedTools?, blockedTools?, resultToMessage? })` lists an MCP server's tools and bridges each into a `VoiceAgentTool`; spreads into `createVoiceAgent({ tools })`. Structural `MCPClientLike` (`listTools` + `callTool`) — any `@modelcontextprotocol/sdk` transport plugs in, no SDK bundled. Flattens text content blocks, falls back to structuredContent, surfaces `isError`. Tier-1 audit-#3 picks all closed (simulator .520, prompt-to-pathway .521, MCP .522) |
| Monitoring & Issues (Vapi) | **ALREADY SHIPPED** — `src/voiceMonitoring.ts`: `createVoiceMonitorRunner` (trigger-based rule eval over session/cost/quality metrics), `createVoiceMonitorWebhookNotifier`, issue stores, and monitor routes. No new work needed for tier 2 — this was already in place from the observability batch |
| Variable-value analytics (Vapi) | **SHIPPED voice@0.0.22-beta.523** — `buildVoiceVariableAnalytics({ calls, variables, successOutcomes?, topValuesPerVariable? })` groups call metrics (success rate, avg duration, avg cost, outcome histogram) by extracted variable values. Returns `{ totalCalls, overall, byVariable }` with per-value `{ count, share, successRate, avgDurationSeconds, avgCostUsd, outcomes }`, sorted by count, missing-value accounting. Default success outcomes: won/resolved/qualified/completed/booked |
| Zero-Data-Retention mode (Vapi/Bland) | **SHIPPED voice@0.0.22-beta.523** — `createVoiceZeroDataRetentionPolicy({ mode, retain? })` (off/strict/custom) + per-flag retain controls (transcriptText/assistantText/recordings/traceText/metadata). Scrubbers `scrubVoiceTurnForZeroDataRetention` (masks text/transcripts/assistant text, drops attachments, keeps ids/timestamps/citations) and `scrubVoiceTraceForZeroDataRetention` (masks string payload fields, preserves structure). Predicates `shouldRetainVoiceRecording`/`shouldRetainVoiceTranscript`/`isVoiceZeroDataRetentionActive`. **Tier 2 complete** |
