...
const fixtureMatchesCodeSwitchPair = (
	...
	if (pair === 'ca-es') {
		return tags.has('bsc') || tags.has('bsc-lt') || tags.has('bscs-code-switching-ca-es-asr-test');
	}
...
const buildBenchmarkOpenOptions = (
	profile: VoiceBenchmarkProfile
): ((fixture: { language?: string; tags?: string[] }) => Partial<STTAdapterOpenOptions> | undefined) =>
	(fixture) => {
...
const buildBenchmarkLexicon = (
	profile: VoiceBenchmarkProfile
): ((fixture: { expectedText?: string; language?: string; tags?: string[] }) => VoiceLexiconEntry[] | undefined) =>
	(fixture) => {
...
const buildBenchmarkPhraseHints = (
	profile: VoiceBenchmarkProfile
): ((fixture: { expectedText?: string; expectedTerms?: string[]; language?: string }) => VoicePhraseHint[] | undefined) =>
	(fixture) => {
		if (!isCodeSwitchProfile(profile)) {
			return undefined;
		}
...
const applyTargetCorrections = (
	...
	if (target === 'deepgram-corrected') {
		return withCorrectedAdapterId(target, applyCorrectedBenchmarkReport(report, fixtures));
	}
...
const filterFixturesForProfile = async (
	profile: VoiceBenchmarkProfile
) => {
...
const buildMissingProfileError = (profile: VoiceBenchmarkProfile) => {
	if (profile !== 'multilingual' && !isCodeSwitchProfile(profile)) {
		...
		if (profile === 'multi-speaker-clean' || profile === 'multi-speaker-noisy') {
...
const runOpenAIDiagnosticProbe = async (env: VoiceBenchEnv) => {
	...
	const configuredModels = (env.OPENAI_TEST_MODELS ?? '')
		...
		.filter((value): value is string => value.length > 0);
	const modelsToTry = configuredModels.length > 0 ? configuredModels : targetModels;
	...
	for (let offset = 0; offset < resampledAudio.byteLength; offset += bytesPerChunk) {
		chunks.push(
			resampledAudio.slice(offset, Math.min(offset + bytesPerChunk, resampledAudio.byteLength))
		);
	}
	...
	for (let index = 0; index < modelsToTry.length; index += 1) {
		...
		const logs = {
			...
			errors: [] as Array<{ code?: string; message: string; recoverable: boolean }>,
			finalText: ''
		};
		let connected = false;
...
const main = async () => {
	const positionalArgs = process.argv.slice(2).filter((entry) => !entry.startsWith('--'));
	...
	if (mode === 'diagnostic' && target === 'openai') {
		...
		await clearBenchmarkResultStem(buildResultStem(target, profile, mode, runs, variant));
