...
const asRealtimeSTTAdapter = (adapter: RealtimeAdapter): STTAdapter => ({
	...
	open: async (
		...
		if (sourceFormat.channels !== 1) {
			throw new Error('OpenAI session benchmark resampler supports mono PCM only.');
		}
...
const parseEnv = async (): Promise<VoiceBenchEnv> => {
	...
	return {
		...
		OPENAI_RESPONSE_MODE: normalizeEnvValue(
			values.OPENAI_RESPONSE_MODE
		) as 'text' | 'audio' | undefined,
...
const createSoakScenario = (input: {
	...
	if (!baseFixture) {
		throw new Error(`Unable to build soak scenario "${input.id}" without fixtures.`);
	}

	const segmentByteLengths = input.fixtures.map((fixture) => fixture.audio.byteLength);
...
const loadSoakScenarios = async (
	...
	return [
		createSoakScenario({
			...
			tags: ['clean', 'dialogue-style', 'long-session', 'multi-turn', 'reconnect-churn', 'soak', 'synthetic'],
		...
		createSoakScenario({
			...
			tags: ['challenging', 'dialogue-style', 'long-session', 'multi-turn', 'noisy', 'reconnect-churn', 'soak', 'synthetic'],
		...
		createSoakScenario({
			...
			tags: ['challenging', 'dialogue-style', 'long-session', 'multi-turn', 'noisy', 'reconnect-churn', 'soak', 'stress', 'synthetic'],
...
const main = async () => {
	...
	const profile =
		profileArgumentIndex >= 0 && process.argv[profileArgumentIndex + 1] === 'soak'
			? 'soak'
			: 'baseline';
	const trace = process.argv.includes('--trace');
	const runsArgumentIndex = process.argv.findIndex((value) => value === '--runs');
	const runs =
		runsArgumentIndex >= 0 && process.argv[runsArgumentIndex + 1]
			? Number(process.argv[runsArgumentIndex + 1])
			: 1;
	const variantArgumentIndex = process.argv.findIndex((value) => value === '--variant');
	const variant =
		variantArgumentIndex >= 0 ? process.argv[variantArgumentIndex + 1]?.trim() : undefined;
	...
	const correctionProfileForTarget =
		target === 'deepgram-corrected'
			...
			: target === 'best-stt'
				? 'generic'
				: undefined;
	let scenarios = (await loadScenariosForTarget({
	...
	if (
		!['all', 'best-stt', 'cheap-stt', 'deepgram', 'deepgram-hybrid', 'deepgram-corrected', 'deepgram-corrected-audit', 'deepgram-openai-hybrid', 'deepgram-flux', 'assemblyai', 'openai'].includes(
			target
		)
	) {
		throw new Error(
	...
	const runTarget = async (
		...
		if (adapterId === 'deepgram-corrected') {
			...
			const correctTurn =
				createBenchmarkCorrectionHandler('benchmark-seeded');
		...
		if (adapterId === 'deepgram-corrected-audit') {
			...
			if (runs > 1) {
				throw new Error('deepgram-corrected-audit does not support --runs yet.');
			}
