...
const manifestPath = resolve(resultsDir, 'telephony-live-shootout-manifest.json');
...
for (const variant of variants) {
	...
	if (!(await file.exists())) {
		throw new Error(`Expected live telephony shootout output was not written: ${outputPath}`);
	}

	const metadata = await stat(outputPath);
	if (metadata.mtimeMs < startedAt) {
		throw new Error(`Stale live telephony shootout output detected: ${outputPath}`);
	}
