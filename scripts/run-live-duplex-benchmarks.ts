...
const outputPaths = [
	resolve(resultsDir, 'duplex-live-elevenlabs.json'),
	resolve(resultsDir, 'duplex-live-openai.json')
];
const aggregatePath = resolve(resultsDir, 'duplex-live-all.json');
const manifestPath = resolve(resultsDir, 'duplex-live-run-manifest.json');
...
const runs = [
	{
		args: ['bun', 'run', './scripts/benchmark-live-duplex.ts', 'elevenlabs'],
		label: 'duplex-live-elevenlabs',
		path: outputPaths[0]
	},
	{
		args: ['bun', 'run', './scripts/benchmark-live-duplex.ts', 'openai'],
		label: 'duplex-live-openai',
		path: outputPaths[1]
	}
];
...
const outputs = await Promise.all(
	runs.map(async (run) => {
		...
		if (!(await file.exists())) {
			throw new Error(
				`Expected live duplex benchmark output was not written: ${run.path}`
			);
		}

		const metadata = await stat(run.path);
		if (metadata.mtimeMs < startedAt) {
			throw new Error(`Stale live duplex benchmark output detected: ${run.path}`);
		}
...
if (aggregateMetadata.mtimeMs < startedAt) {
	throw new Error(`Stale live duplex benchmark output detected: ${aggregatePath}`);
}
...
console.log(
	['Saved live duplex benchmark JSON to', ...outputPaths, aggregatePath, manifestPath].join(
		'\n'
	)
);
