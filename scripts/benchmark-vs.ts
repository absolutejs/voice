...
const BENCHMARK_RESULTS_DIR = resolve(import.meta.dir, '..', 'benchmark-results');
...
const PROFILE_FILTERS: Record<Exclude<VsProfile, 'all'>, (tags: string[]) => boolean> = {
	accents: (tags) => tags.some((tag) => tag.includes('accent')),
	baseline: (tags) => !tags.some((tag) => tag.includes('accent')),
	clean: (tags) =>
		tags.some((tag) => tag === 'clean') ||
		(!tags.some((tag) => ['accent', 'accent-noisy', 'noisy', 'synthetic-noise', 'stress'].includes(tag))),
	'code-switch': (tags) =>
		tags.some((tag) => tag === 'code-switch' || tag === 'code_switch'),
	jargon: (tags) => tags.some((tag) => tag === 'jargon' || tag === 'domain-heavy'),
	multilingual: (tags) =>
		tags.some((tag) => tag === 'multilingual' || tag === 'international' || tag === 'code-switch' || tag === 'code_switch'),
...
const compareWithCompetitors = (entries: VsComparativeEntry[]) => {
	...
	const best = voiceEntries.reduce<VsComparativeEntry | null>((winning, current) => {
		if (!winning) {
			return current;
		}
	...
	const vapiEntries = competitorEntries.filter((entry) => entry.source === 'vapi');
	const bestVapi = vapiEntries.reduce<VsComparativeEntry | null>((winner, current) => {
		if (!winner) {
			return current;
		}
	...
	return {
		...
		vapiDelta: {
			passRateDeltaVsVapi:
				best.summary.passRate - bestVapi.summary.passRate,
			wordAccuracyRate: best.summary.wordAccuracyRate - bestVapi.summary.wordAccuracyRate,
			termRecall: best.summary.averageTermRecall - bestVapi.summary.averageTermRecall,
...
const renderMarkdownSummary = (input: {
	...
	lines.push('');
	lines.push(`- Generated: ${new Date(input.output.generatedAt).toISOString()}`);
	lines.push(`- Profile: \`${input.output.profile}\``);
	lines.push(`- Targets: ${input.output.targets.map((target) => `\`${target}\``).join(', ')}`);
	...
	lines.push('');
	lines.push('| Adapter | Score | Pass Rate | Accuracy | Term Recall | Acceptance |');
	lines.push('| --- | ---: | ---: | ---: | ---: | --- |');
	...
	if (input.competitors.length === 0) {
		...
		lines.push('| Competitor | Pass Delta Vs Voice | Accuracy | Term Recall | Latency |');
		lines.push('| --- | ---: | ---: | ---: | ---: |');
	...
	lines.push(
		`- Compared voice adapters: ${input.output.entries.filter((entry) => entry.source === 'voice').map((entry) => `\`${entry.adapterId}\``).join(', ')}`
	);
...
const normalizeCompetitorSummary = (
	...
	const averageTermRecall = normalizeRange(entry.summary.averageTermRecall) ?? passRate;
	...
	const normalizedWordAccuracyRate = wordAccuracyRate ?? (averageWordErrorRate !== undefined ? clamp(1 - averageWordErrorRate) : undefined);
...
const main = async () => {
	...
	if (reports.length === 0) {
		...
		throw new Error(`No benchmarkable adapters are configured: missing ${needs}.`);
	}
	...
	const rankedComparativeEntries = [...entries, ...competitorEntries].map((entry) => ({
		...entry,
		// include derived ranking score for easy sort in consumers
		rank: compareScore(entry.summary)
	})) as (VsComparativeEntry & { rank: number })[];

	const ranked = [...rankedComparativeEntries].sort((left, right) => right.rank - left.rank);
	const best = ranked[0];
	...
	if (requestedProfile === 'accents' && best && best.summary.wordAccuracyRate < 0.65) {
		process.exitCode = 1;
	}
};
