...
type BrowserDuplexSingleReport = {
	fixtures: Array<{
		adapterId: string;
...
if (!Number.isFinite(runs) || runs <= 0) {
	throw new Error(`Invalid browser duplex series run count: ${process.argv[2] ?? ''}`);
}
	...
	return [
		{
			...
			label: `duplex-browser-elevenlabs-series-run-${runNumber}`,
			outputPath: resolve(
				resultsDir,
				`duplex-browser-elevenlabs-series-run-${runNumber}.json`
			),
			runNumber
		},
		{
			adapterId: 'openai',
			label: `duplex-browser-openai-series-run-${runNumber}`,
			outputPath: resolve(
				resultsDir,
				`duplex-browser-openai-series-run-${runNumber}.json`
			),
			runNumber
		}
	];
...
const summaryPath = resolve(
	resultsDir,
	`duplex-browser-series-summary-runs-${runs}.json`
);

await Promise.all(
	[summaryPath, ...descriptors.map((descriptor) => descriptor.outputPath)].map((path) =>
		rm(path, { force: true })
	)
);
...
const children = descriptors.map((descriptor) =>
	Bun.spawn({
		cmd: [
			'bun',
			'run',
			'./scripts/benchmark-browser-duplex.ts',
...
const outputs = await Promise.all(
	descriptors.map(async (descriptor) => {
		...
		if (!(await file.exists())) {
			throw new Error(
				`Expected browser duplex series output was not written: ${descriptor.outputPath}`
			);
		}
		...
		if (metadata.mtimeMs < startedAt) {
			throw new Error(
				`Stale browser duplex series output detected: ${descriptor.outputPath}`
			);
		}
		...
		return {
			...
			report: (await file.json()) as BrowserDuplexSingleReport,
...
const summarizeTarget = (target: string) => {
	...
	const sessionCloseSamples = fixtures.filter(
	...
	return {
		...
		stable: passCount === fixtures.length && fixtures.length > 0
	};
};
...
if (summaryMetadata.mtimeMs < startedAt) {
	throw new Error(`Stale browser duplex summary output detected: ${summaryPath}`);
}

console.log(
	['Saved browser duplex series JSON to', summaryPath].join('\n')
);
