...
if (!Number.isFinite(runs) || runs <= 0) {
  throw new Error(
    `Invalid browser duplex series run count: ${process.argv[2] ?? ""}`,
  );
}
...
if (summaryMetadata.mtimeMs < startedAt) {
  throw new Error(
    `Stale browser duplex summary output detected: ${summaryPath}`,
  );
}

console.log(["Saved browser duplex series JSON to", summaryPath].join("\n"));
