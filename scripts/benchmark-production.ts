...
const asRealtimeSTTAdapter = (adapter: RealtimeAdapter): STTAdapter => ({
	...
	open: async (
		...
		if (sourceFormat.channels !== 1) {
			throw new Error('OpenAI production benchmark resampler supports mono PCM only.');
		}
...
const summarizeTarget = async (
	...
	const allFixtures = profileResults.flatMap((result) => result.report.fixtures);
...
const buildProductionCorrectionAudit = async (
	...
	const sliceHoldout = (report: VoiceSTTBenchmarkReport): VoiceSTTBenchmarkReport => {
	...
	const lexicalHoldoutFixtureIds = Array.from(
		new Set(profileAudits.flatMap((profile) => profile.lexicalHoldout.fixtureIds))
	);
	...
	return {
		...
		overall: {
			...
			holdout: {
				...
				raw: summarizeCorrectionAudit(sliceHoldout(overallRaw), OVERALL_ACCEPTANCE)
			},
			lexicalHoldout: {
				benchmarkSeeded: summarizeCorrectionAudit(
...
const main = async () => {
	...
	if (targetArg === 'deepgram-corrected-audit') {
		const output = await buildProductionCorrectionAudit(fixtures, profiles, env);
	...
	if (
		results.some(
			(result) =>
				result.failure ||
				(result.overall && !result.overall.acceptance.passed)
		)
	) {
		process.exitCode = 1;
	}
