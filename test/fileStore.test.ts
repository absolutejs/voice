...
test("createVoiceFileRuntimeStorage exposes persistent sessions and reviews", async () => {
  ...
  expect(
    (await secondRuntimeStorage.session.get("session-2"))?.lastActivityAt,
  ).toBe(2222);
  expect(
...
test("createVoiceFileIncidentBundleStore persists and filters incident artifacts", async () => {
  ...
  expect(
    (await secondStore.list({ expiredAt: 160 })).map((item) => item.id),
  ).toEqual(["incident-file"]);
...
test("createVoiceFileRuntimeStorage exposes persistent tasks and events", async () => {
  ...
  expect(
    (await secondRuntimeStorage.traces.list({ sessionId: "session-2" }))[0]
      ?.type,
  ).toBe("agent.result");
