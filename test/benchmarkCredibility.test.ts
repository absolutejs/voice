...
test("aggregate accuracy distinguishes macro and micro WER", () => {
  const values = [
    scoreTranscriptAccuracy("a", "a"),
    scoreTranscriptAccuracy("wrong", "one two three"),
  ];
...
test("paired bootstrap is deterministic and preserves direction", () => {
  const result = comparePairedMetrics([0.4, 0.3, 0.2], [0.2, 0.1, 0.1], {
    samples: 1_000,
    seed: 7,
  });
...
test("benchmark artifacts are checksummed", () => {
  const manifest = {
    adapter: { id: "example" },
    corpus: { fixtures: [], manifestSha256: "abc", name: "test", version: "1" },
    createdAt: "2026-07-22T00:00:00.000Z",
    environment: {},
    git: {},
    preprocessing: {},
    promptTrack: "unprompted" as const,
    seed: 1,
  };
  const artifact = buildVoiceBenchmarkArtifact(manifest, { passRate: 1 });
  expect(verifyVoiceBenchmarkArtifact(artifact)).toBe(true);
  expect(
    verifyVoiceBenchmarkArtifact({ ...artifact, report: { passRate: 0 } }),
  ).toBe(false);
});
