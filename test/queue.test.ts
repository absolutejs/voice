...
test("createVoiceIntegrationSinkWorker retries pending sink deliveries and dead-letters exhausted failures", async () => {
  ...
  expect(
    events.get("event-sink-pending")?.sinkDeliveries?.helpdesk?.status,
  ).toBe("delivered");
  expect(
    deadLetters.get("event-sink-failed")?.sinkDeliveries?.helpdesk?.attempts,
  ).toBe(3);
});
...
test("createVoiceTraceSinkDeliveryWorker retries queued trace deliveries and dead-letters failures", async () => {
  ...
  expect(
    await summarizeVoiceTraceSinkDeliveries(await deliveries.list(), {
      deadLetters,
    }),
  ).toMatchObject({
    deadLettered: 1,
