...
test("createTelephonyVoiceTestFixtures derives narrowband phone-style fixtures", async () => {
  ...
  expect(
    telephonyFixtures.every((fixture) => fixture.id.endsWith("-telephony")),
  ).toBe(true);
  expect(
...
test("createMultiSpeakerVoiceTestFixtures derives synthetic handoff fixtures", async () => {
  ...
  expect(
    multiSpeakerFixtures.every((fixture) =>
      fixture.tags?.includes("multi-speaker"),
    ),
  ).toBe(true);
  expect(
    multiSpeakerFixtures.every(
      (fixture) => fixture.expectedSpeakerTurns?.length,
    ),
  ).toBe(true);
...
test("createJargonVoiceTestFixtures derives domain-heavy fixtures from bundled audio", async () => {
  ...
  expect(
    jargonFixtures.every((fixture) => fixture.id.endsWith("-jargon")),
  ).toBe(true);
  expect(
    jargonFixtures.every((fixture) => fixture.tags?.includes("jargon")),
  ).toBe(true);
  expect(
...
test("runSTTAdapterBenchmark post-clusters synthetic handoff speaker reentry when overlap is low", async () => {
  const fixture = {
    ...
    expectedText:
      "alpha bravo charlie delta echo rainstorms around the mountain",
    format: {
      channels: 1 as const,
      container: "raw" as const,
      encoding: "pcm_s16le" as const,
...
test("generic phrase hints derive name corrections without seeded noisy phrases", async () => {
  const fixtures = await loadVoiceTestFixtures();
  const routeFixture = fixtures.find(
    (fixture) => fixture.id === "traveled-back-route-clean",
  );
  ...
  expect(
    phraseHints.some((hint) => (hint.aliases ?? []).includes("beneath wealth")),
  ).toBe(false);
});
...
test("code-switch benchmark lexicon caps low-signal entries while keeping mixed-script phrases", () => {
  ...
  expect(lexicon.some((entry) => entry.text === "complain कर लो")).toBe(true);
  expect(lexicon.some((entry) => entry.text === "future opportunities")).toBe(
    true,
  );
...
test("code-switch benchmark lexicon adds parlament-specific recovery aliases", () => {
  ...
  const sloganEntry = lexicon.find(
    (entry) => entry.text === "espanya es paro y muerte",
  );
...
test("correction benchmark audit exposes a lexical holdout that excludes seeded noisy phrase families", async () => {
  ...
  expect(audit.holdout.fixtureIds).toEqual([
    "dialogue-two-clean",
    "dialogue-two-noisy",
  ]);
...
test("session correction audit exposes lexical holdout independently from scenario holdout", () => {
  ...
  const raw = {
    ...
    generatedAt: Date.now(),
    scenarios: [
      createScenario("dialogue-two-clean", true),
      createScenario("dialogue-two-noisy", false),
    ],
    summary: {
      adapterId: "deepgram-flux",
