...
const createCampaignStore = (
  ...
  const stored = new Map(records.map((record) => [record.campaign.id, record]));
...
test("buildVoiceDataRetentionPlan dry-runs retention across runtime stores", async () => {
  ...
  expect(report.scopes.flatMap((scope) => scope.deletedIds).sort()).toEqual([
    "event-old",
    "review-old",
    "session-old",
    "task-old",
...
test("createVoiceDataControlRoutes exposes report audit export and guarded retention apply", async () => {
  ...
  const reportResponse = await routes.handle(
    new Request(
      "http://voice.test/data-control.json?beforeOrAt=200&scopes=traces",
    ),
  );
