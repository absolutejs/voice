...
test("voice ops task policies apply default SLA and priority by disposition", () => {
  ...
  expect(
    resolveVoiceOpsTaskAgeBucket(updated, { at: updated.dueAt! - 1 }),
  ).toBe("due-soon");
});
...
test("summarizeVoiceOpsTaskAnalytics reports assignee throughput and worker activity", () => {
  ...
  expect(
    analytics.workers.find((worker) => worker.workerId === "worker-a")
      ?.totalClaims,
  ).toBe(2);
  expect(
    analytics.workers.find((worker) => worker.workerId === "worker-a")?.failed,
  ).toBe(1);
  expect(
    analytics.workers.find((worker) => worker.workerId === "worker-b")
      ?.completed,
  ).toBe(1);
  expect(analytics.agingBuckets.some(([bucket]) => bucket === "overdue")).toBe(
    true,
  );
});
...
test("deliverVoiceIntegrationEvent posts the event and records delivery metadata", async () => {
  ...
  expect(requests[0]?.headers.get("x-absolutejs-signature")).toMatch(
    /^sha256=/,
  );
