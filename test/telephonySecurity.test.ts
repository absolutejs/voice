...
const createTempSQLitePath = () =>
  join(
    tmpdir(),
    `absolutejs-voice-webhook-security-${crypto.randomUUID()}.sqlite`,
  );
...
test("createVoiceTelephonyWebhookSecurityPreset verifies Twilio and exposes idempotency", async () => {
  ...
  expect(
    await preset.twilio.idempotency.store.get("twilio:call-1"),
  ).toMatchObject({
    event: {
      provider: "twilio",
    },
...
test("SQLite telephony webhook security preset persists replay claims", async () => {
  ...
  expect(await secondPreset.plivo.nonceStore.claim?.("sqlite-plivo")).toBe(
    false,
  );
  expect(await firstPreset.telnyx.eventStore.claim?.("sqlite-telnyx")).toBe(
    true,
  );
...
test("telephony webhook security routes expose the readiness report", async () => {
  ...
  const response = await app.handle(
    new Request(
      "https://voice.example.test/api/voice/telephony/webhook-security",
    ),
  );
  ...
  expect(
    body.providers.find((provider) => provider.provider === "twilio"),
  ).toMatchObject({
    enabled: true,
    status: "pass",
  });
