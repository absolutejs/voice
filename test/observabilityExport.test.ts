...
const createFakeObservabilityS3Client = () => {
  const objects = new Map<
    string,
    { data: string | Uint8Array; type?: string }
  >();
...
test("validateVoiceObservabilityExportRecord validates customer-ingested export records", async () => {
  ...
  const artifactIndex = await readObservabilityExportFixture(
    "artifact-index.json",
  );
  ...
  expect(report.issues).toEqual(
    expect.arrayContaining([
      ...
      expect.stringContaining(
        "latest successful observability export delivery age",
      ),
...
test("replayVoiceObservabilityExport reads delivered file export and receipt health", async () => {
  ...
  const replay = await replayVoiceObservabilityExport({
    ...
    kind: "records",
    manifest: JSON.parse(
      await Bun.file(join(dir, "run-1", "manifest.json")).text(),
    ),
  });
...
test("createVoiceObservabilityExportRoutes exposes artifact index and downloads", async () => {
  ...
  expect(await index.json()).toMatchObject({
    artifacts: [
      expect.objectContaining({
        downloadHref: "/api/voice/observability-export/artifacts/proof-pack",
        id: "proof-pack",
        required: true,
      }),
    ],
...
test("createVoiceObservabilityExportRoutes can serve a supplied artifact index without rebuilding the report", async () => {
  const app = createVoiceObservabilityExportRoutes({
    ...
    store: {
      ...
      list: async () => {
        throw new Error(
          "report should not be built for supplied artifact index",
        );
      },
      remove: async () => {},
    },
...
test("deliverVoiceObservabilityExport writes manifest, index, and artifacts to S3 destination", async () => {
  ...
  expect(
    s3.objects.get("absolutejs/demo/run-1/artifacts/proof-pack.md")?.type,
