...
test("createVoiceIncidentTimelineRoutes exposes json html and markdown", async () => {
  ...
  const html = await routes.handle(
    new Request("http://localhost/voice/incidents"),
  );
...
test("createVoiceIncidentTimelineRoutes exposes and executes recovery actions", async () => {
  ...
  const run = await routes.handle(
    new Request(
      "http://localhost/api/voice/incident-timeline/actions/proof.rerun",
      {
        method: "POST",
      },
    ),
  ...
  const blocked = await routes.handle(
    new Request(
      "http://localhost/api/voice/incident-timeline/actions/support.open",
      {
        method: "POST",
      },
    ),
...
test("buildVoiceIncidentRecoveryOutcomeReport summarizes audited incident action impact", async () => {
  ...
  const json = await routes.handle(
    new Request(
      "http://localhost/api/voice/incident-timeline/recovery-outcomes",
    ),
  );
  const html = await routes.handle(
...
test("buildVoiceIncidentRecoveryTrendSLOReadinessCheck passes for healthy recovery trends", () => {
  const check = buildVoiceIncidentRecoveryTrendSLOReadinessCheck(
    {
      checkedAt: 1_000,
      summary: {
        cycles: 2,
    ...
    },
    {
      minActions: 1,
      minCycles: 1,
      minImprovementRate: 0.4,
    },
  );

  expect(check.status).toBe("pass");
  expect(check.label).toBe("Incident recovery trend SLO");
  expect(check.value).toContain("50% improved");
});

test("buildVoiceIncidentRecoveryTrendSLOReadinessCheck warns on insufficient trend data", () => {
  const check = buildVoiceIncidentRecoveryTrendSLOReadinessCheck(
    {
      checkedAt: 1_000,
      summary: {
  ...
  expect(check.gateExplanation).toBeUndefined();
  expect(check.detail).toContain("empty");
});

test("buildVoiceIncidentRecoveryTrendSLOReadinessCheck fails on hard recovery budgets", () => {
  const check = buildVoiceIncidentRecoveryTrendSLOReadinessCheck(
    {
      checkedAt: 1_000,
      summary: {
  ...
  expect(check.gateExplanation?.observed).toContain("%");
});
