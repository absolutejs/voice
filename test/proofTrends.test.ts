...
describe("proof trends", () => {
  ...
  test("buildVoiceProofTrendRecommendationReport turns sustained history into provider and runtime guidance", () => {
    ...
    expect(
      recommendations.bestProviders.map((provider) => provider.role),
    ).toEqual(["llm", "stt", "tts"]);
    ...
    expect(
      recommendations.profiles.every((profile) => profile.status === "pass"),
    ).toBe(true);
    ...
    expect(recommendations.recommendations.map((item) => item.surface)).toEqual(
      ["provider-path", "runtime-channel", "live-latency", "turn-latency"],
    );
  });
  ...
  test("buildVoiceProofTrendProfileSummaries aggregates profile history before using fallback derivation", () => {
    ...
    const supportAgent = profiles.find(
  ...
  test("buildVoiceRealCallProfileEvidenceFromReconnectProofReports feeds reconnect proof into profile history", () => {
    const evidence = buildVoiceRealCallProfileEvidenceFromReconnectProofReports(
      {
        checkedAt: Date.UTC(2026, 3, 30, 12, 0, 0),
        contract: {
          checkedAt: Date.UTC(2026, 3, 30, 12, 0, 0),
          issues: [],
          pass: true,
        ...
        status: "pass",
        summary:
          "Checked 1 completed session and 2 reconnect snapshots. Resume was observed.",
      },
      {
        profileId: "meeting-recorder",
    ...
    const history = buildVoiceRealCallProfileHistoryReport({
      ...
      profiles: [{ id: "meeting-recorder", label: "Meeting recorder" }],
    });
    ...
    expect(history.summary.maxReconnectP95Ms).toBe(420);
    expect(history.summary.reconnect).toMatchObject({
      resumeLatencyP95Ms: 420,
      reconnected: true,
      resumed: true,
      status: "pass",
  ...
  test("buildVoiceRealCallProfileReadinessCheck gates required real-call profile evidence", async () => {
    ...
    expect(
      buildVoiceRealCallProfileRecoveryActions(history, {
        ...
        requiredProfileSurfaces: ["browser", "reconnect"],
        requiredProviderRoles: ["llm", "stt", "tts"],
      ...
      "Run reconnect profile proof for custom-profile",
    ...
    expect(
      json.defaults.profiles.some(
        (profile: { profileId?: string }) =>
          profile.profileId === "support-agent",
      ),
    ).toBe(true);
  ...
  test("createVoiceRealCallProfileRecoveryActionRoutes exposes actions and executable handlers", async () => {
    ...
    const app = createVoiceRealCallProfileRecoveryActionRoutes({
      ...
      requiredProfileIds: ["support-agent"],
      requiredProfileSurfaces: ["reconnect"],
      source: {
        evidence: [
          {
            generatedAt: "2026-04-29T12:00:00.000Z",
    ...
    const actionsResponse = await app.handle(
      new Request(
        "http://localhost/api/voice/real-call-profile-history/actions",
      ),
    );
    ...
    const reconnectResponse = await app.handle(
      new Request(
        "http://localhost/api/voice/real-call-profile-history/collect-reconnect-proof",
        { method: "POST" },
      ),
    );
    ...
    expect(
      actions.actions.map(
        (action: { href: string; method?: string }) => action.href,
      ),
    ).toContain(
    ...
    expect(
      ...
      "/api/voice/real-call-profile-history/collect-reconnect-proof?profileId=support-agent",
    );
    ...
    expect(reconnectResponse.status).toBe(501);
    expect(reconnect).toMatchObject({
      actionId: "collect-reconnect-proof",
      ok: false,
      status: "fail",
    });
  ...
  test("createVoiceRealCallProfileRecoveryActionRoutes can queue and poll recovery jobs", async () => {
    ...
    const jobsResponse = await app.handle(
      new Request(
        "http://localhost/api/voice/real-call-profile-history/actions/jobs",
      ),
    );
  ...
  test("runVoiceRealCallProfileRecoveryLoop runs real-call recovery actions in parallel", async () => {
    ...
    expect(report.jobs.map((job) => job.result.status)).toEqual([
      "pass",
      "pass",
    ]);
  ...
  test("runVoiceRealCallProfileRecoveryLoop repeats bounded passes until the gate passes", async () => {
    ...
    const fakeFetch: typeof fetch = async (input, init) => {
      ...
      if (
        url.pathname === "/recover/browser" ||
        url.pathname === "/recover/phone"
      ) {
        return Response.json({
      ...
      if (
        url.pathname.startsWith("/api/voice/real-call-profile-history/actions")
      ) {
        return Response.json({
  ...
  test("appendVoiceRealCallProfileRecoveryEvidence emits profiled provider and runtime traces", async () => {
    ...
    expect(
      result.events.every(
        (event) => event.metadata?.profileId === "support-agent",
      ),
    ).toBe(true);
  ...
  test("buildVoiceRealCallProfileEvidenceFromTraceEvents ignores turn-only profiled sessions", () => {
    ...
    expect(evidence[0]?.surfaces).toContain("reconnect");
  });
