...
describe("telephony media routes", () => {
  test("proves carrier media packets parse into generic media frames", () => {
    ...
    expect(
      report.carriers.every((carrier) => carrier.frame?.source === "telephony"),
    ).toBe(true);
    expect(
      report.carriers.every((carrier) => carrier.lifecycle.status === "pass"),
    ).toBe(true);
    expect(report.carriers.every((carrier) => carrier.lifecycle.started)).toBe(
      true,
    );
    expect(report.carriers.every((carrier) => carrier.lifecycle.stopped)).toBe(
      true,
    );
  });
