...
const carrierAnswerLabel = (provider: VoicePhoneAgentCarrierSummary['provider']) =>
	provider === 'telnyx'
		? 'TeXML URL'
		: provider === 'plivo'
			? 'Answer URL'
...
export const evaluateVoicePhoneAssistantEvidence = (
	...
	const issues: string[] = [];
	const providers = uniqueSorted(report.carriers.map((carrier) => carrier.provider));
	...
	const readyCarriers =
		...
		report.carriers.filter((carrier) =>
			findCarrierMatrixEntry(report.matrix, carrier)?.ready
		).length;
	...
	for (const stage of findMissing(lifecycleStages, input.requiredLifecycleStages)) {
...
export const evaluateVoicePhoneCallControlEvidence = (
	...
	const providers = uniqueSorted(
		productionSmokes
			...
			.filter((provider): provider is VoiceTelephonyProvider => Boolean(provider))
	);
