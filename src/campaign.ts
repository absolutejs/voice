...
export type VoiceCampaignStore = {
  get: (
    ...
    | Promise<VoiceCampaignRecord | undefined>
    | VoiceCampaignRecord
    | undefined;
...
export type VoiceCampaignRuntime = {
  ...
  importRecipients: (
    ...
    VoiceCampaignRecord & { import: VoiceCampaignRecipientImportResult }
  >;
...
export type VoiceCampaignTelephonyOutcomeRecorderRecordInput<
  TResult = unknown,
...
const isWithinCampaignTimeWindow = (
...
const getCampaignRetryBackoffMs = (
  ...
  if (Array.isArray(backoff)) {
    return Math.max(
      0,
      backoff[Math.min(backoff.length - 1, attemptNumber - 1)] ?? 0,
    );
  }
...
const parseVoiceCampaignCSVRows = (
  csv: string,
  ...
  for (const record of records) {
    ...
    for (const attempt of record.attempts) {
      ...
      if (
        ...
        attempt.updatedAt >= rateWindowStart
      ) {
    ...
    if (options.leases) {
      const lease = await options.leases.get(
        getCampaignLeaseTaskId(campaignId),
      );
      if (lease) {
...
export const createVoiceCampaign = (
  ...
  return {
    ...
    tick: async (campaignId) => {
      ...
      if (
        schedule?.quietHours &&
        isWithinCampaignTimeWindow(schedule.quietHours, at)
      ) {
        result.blocked.push({
          reason: "quiet-hours",
        });
...
export const createVoiceCampaignWorker = (
  ...
  const maxCampaigns = Math.max(
    1,
    options.maxCampaigns ?? Number.MAX_SAFE_INTEGER,
  );
  ...
  return {
    drain: async () => {
      ...
      const campaigns = [...(await runtime.list())]
        ...
        .sort(
          (left, right) => left.campaign.createdAt - right.campaign.createdAt,
        )
...
const firstOutcomeString = (values: Array<unknown>): string | undefined => {
...
export const applyVoiceCampaignTelephonyOutcome = async <TResult = unknown>(
  ...
  const campaignId =
    (await options.resolveCampaignId?.(input)) ?? defaults.campaignId;
  const attemptId =
    (await options.resolveAttemptId?.(input)) ?? defaults.attemptId;
  ...
  if (
    match.attempt.status === "failed" ||
    match.attempt.status === "succeeded"
  ) {
    return {
      applied: false,
...
export const createVoiceCampaignTelephonyOutcomeHandler =
  <TResult = unknown>(options: VoiceCampaignTelephonyOutcomeOptions<TResult>) =>
...
export const runVoiceCampaignProof = async (
  ...
  if (!final) {
    throw new Error(
      "Campaign proof did not persist the final campaign record.",
    );
  }
...
recipient-no-consent,Barbara,+15550001004,no,delta`,
  ...
  if (!finalScheduled || !finalRetry) {
    throw new Error(
      "Campaign readiness proof did not persist campaign records.",
    );
  }

  pushCampaignReadinessCheck(
    checks,
    "recipient-import-validation",
    imported.import.accepted.length === 1 &&
      imported.import.rejected.length === 3,
    {
  ...
  pushCampaignReadinessCheck(
    ...
    windowBlocked.blocked.some(
      (block) => block.reason === "outside-attempt-window",
    ),
    {
  ...
  pushCampaignReadinessCheck(
    checks,
    "quiet-hours-block",
    quietHours.blocked.some((block) => block.reason === "quiet-hours"),
    {
  ...
  pushCampaignReadinessCheck(
    checks,
    "rate-limit-block",
    rateLimited.blocked.some((block) => block.reason === "rate-limit"),
    {
  ...
  pushCampaignReadinessCheck(
    checks,
    "retry-backoff-block",
    retryInitial.attempted === 1 &&
      retryBackoff.blocked.some((block) => block.reason === "retry-backoff"),
    {
  ...
  pushCampaignReadinessCheck(
    ...
    retryAllowed.attempted === 1 &&
      finalRetry.recipients[0]?.status === "failed",
    {
...
export const renderVoiceCampaignsHTML = (
  ...
  const rows = records
    .map(
      (record) =>
        `<tr><td>${escapeHtml(record.campaign.name)}</td><td>${escapeHtml(record.campaign.status)}</td><td>${String(record.recipients.length)}</td><td>${String(record.attempts.length)}</td><td>${new Date(record.campaign.updatedAt).toLocaleString()}</td></tr>`,
    )
    .join("");
...
export const createVoiceCampaignRoutes = (
  options: VoiceCampaignRoutesOptions,
) => {
  ...
  const app = new Elysia({ name: options.name ?? "absolutejs-voice-campaigns" })
    ...
    .post(
      `${path}/:campaignId/recipients/import`,
    ...
    .post(
      `${path}/:campaignId/attempts/:attemptId/result`,
      async ({ params, request }) =>
