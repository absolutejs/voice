...
const summarizeKind = (
  ...
  const issues = [
    (required || kindEvents.length > 0) &&
    latencies.length < thresholds.minSamples
      ? {
    ...
    issueFromMetric(
      kind,
      "provider_slo.average_latency",
      metrics.averageElapsedMs,
    ),
...
export const evaluateVoiceProviderSloEvidence = (
  ...
  const fallbacks = kindReports.reduce(
    (total, kind) => total + kind.fallbacks,
    0,
  );
  const timeouts = kindReports.reduce(
    (total, kind) => total + kind.timeouts,
    0,
  );
