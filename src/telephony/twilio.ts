...
>(input: {
  ...
  checks.push(
    createSmokeCheck(
      ...
      streamUrl
        ? "TwiML includes a media stream URL."
        : 'TwiML is missing <Stream url="...">.',
      {
...
>(
  ...
  const appendTelephonyMediaTrace = async (
    ...
    const sessionId =
      ...
      (message.event === "start"
        ...
        : undefined) ??
      (message.event === "start" ? message.start.streamSid : "telephony-media");
    const streamSid =
      override?.streamSid ??
      (message.event === "start"
  ...
  const ensureSession = async () => {
    ...
    const voiceSocket = createTwilioSocketAdapter<TResult>(
      socket,
      () => bridgeState,
    );
