...
export const createVoiceTelephonyCarrierMatrix = (
	...
	const entries = options.providers.map((provider) => {
		...
		const failures =
			provider.smoke?.checks.filter((check) => check.status === 'fail').length ??
			0;
		const warnings =
...
${matrix.entries
	.map(
		(entry) => `<article style="border:1px solid #d9e2ec; border-radius:18px; padding:18px; background:#fff; box-shadow:0 18px 48px rgba(15,23,42,.08);">
