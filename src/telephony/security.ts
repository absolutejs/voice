...
const createStores = <TResult>(
  ...
  return {
    ...
    plivo:
      options.plivo?.nonceStore ?? createMemoryVoicePlivoWebhookNonceStore(),
    telnyx:
...
const providerStatus = (
  ...
  if (input.enabled && !input.checks.persistentStore) {
    issues.push(
      "Webhook security store is in-memory; use SQLite, Postgres, or Redis for production.",
    );
  }
...
export const buildVoiceTelephonyWebhookSecurityReport = <TResult = unknown>(
  ...
  const failed = enabled.filter(
    (provider) => provider.status === "fail",
  ).length;
  const warned = enabled.filter(
    (provider) => provider.status === "warn",
  ).length;
  const passed = enabled.filter(
    (provider) => provider.status === "pass",
  ).length;
  const status =
    failed > 0 ? "fail" : warned > 0 || enabled.length === 0 ? "warn" : "pass";
