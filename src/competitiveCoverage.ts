...
export type VoiceCompetitiveCoverageReport = {
  ...
  surfaces: Array<
    VoiceCompetitiveSurface & { status: VoiceCompetitiveCoverageStatus }
  >;
  vapiCoverageEstimate: string;
};
...
export type VoiceCompetitiveCoverageRoutesOptions = {
  ...
  source:
    | (() =>
        | Promise<
            VoiceCompetitiveCoverageReport | VoiceCompetitiveCoverageReportInput
          >
...
export const evaluateVoiceCompetitiveCoverage = (
  ...
  if ((input.requirePass ?? false) && report.status !== "pass") {
    issues.push(
      `Expected competitive coverage to pass, found ${report.status}.`,
    );
  }
  if (
    input.minSurfaces !== undefined &&
    report.summary.surfaces < input.minSurfaces
  ) {
    issues.push(
  ...
  if (
    input.maxMissingSurfaces !== undefined &&
    missing > input.maxMissingSurfaces
  ) {
    issues.push(
...
export const createVoiceCompetitiveCoverageRoutes = (
  ...
  if (markdownPath !== false) {
    app.get(
      ...
      async () =>
        new Response(
          renderVoiceCompetitiveCoverageMarkdown(await report(), title),
          {
            headers: {
