...
const normalizeSample = (
  ...
  if ("summary" in input) {
    return {
      ...
      reconnectP95Ms:
        input.summary.maxReconnectP95Ms ??
        input.summary.reconnect?.resumeLatencyP95Ms,
      runId: input.runId,
  ...
  const recommendedMs =
    baselineP95Ms === undefined
      ...
      : roundMs(
          Math.max(
            options.minimumMs,
            baselineP95Ms * options.headroomMultiplier,
          ),
        );
  const warnAfterMs =
...
export const buildVoiceSloCalibrationReport = (
  ...
  const warnRatio =
    finiteNumber(options.warnRatio) &&
    options.warnRatio > 0 &&
    options.warnRatio < 1
      ? options.warnRatio
      : DEFAULT_WARN_RATIO;
  ...
  const thresholds: VoiceSloCalibrationThresholds = {
    interruption: createThreshold(
      "interruption",
      valuesFor("interruptionP95Ms"),
      {
        headroomMultiplier,
...
export const createVoiceSloThresholdProfile = (
  ...
  const interruptionFailAfterMs = thresholdValue(
    report.thresholds.interruption,
  );
