  ...
  if (typeof options.keepNewest === "number" && options.keepNewest >= 0) {
    const newestIds = new Set(
      [...candidates]
        .sort(
          (left, right) =>
            right.at - left.at || right.id.localeCompare(left.id),
        )
...
const getTaskRetentionRecord = (
  record: StoredVoiceOpsTask,
): RetentionRecord => ({
...
export const applyVoiceDataRetentionPolicy = async (
  ...
  const report = {
    checkedAt: Date.now(),
    deletedCount: reports.reduce(
      (total, report) => total + report.deletedCount,
      0,
    ),
    dryRun,
    scopes: reports,
...
const parseRetentionScopes = (
  value: unknown,
): VoiceDataRetentionScope[] | undefined => {
...
export const evaluateVoiceDataControlEvidence = (
  ...
  const requireZeroRetentionAvailable =
    input.requireZeroRetentionAvailable ?? true;
  ...
  if (requireSelfHostedStorage) {
    const nonSelfHostedConfigured =
      configuredStorage.length - selfHostedConfiguredStorage.length;
  ...
  if (
    missingConfiguredStorage.length > (input.maxMissingConfiguredStorage ?? 0)
  ) {
    issues.push(
  ...
  for (const scope of findMissing(
    retentionScopes,
    input.requiredRetentionScopes,
  )) {
