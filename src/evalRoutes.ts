...
const buildTrend = (
  ...
  for (const session of sessions) {
    const endedAt =
      session.endedAt ?? session.startedAt ?? session.quality.checkedAt;
...
export const runVoiceSessionEvals = async (
  ...
  const failed = limitedSessions.filter(
...
const evaluateScenarioSession = (
  ...
  if (scenario.minTurns !== undefined && turnCount < scenario.minTurns) {
    issues.push(
      `Expected at least ${scenario.minTurns} turn(s), saw ${turnCount}.`,
    );
  }
  if (
  ...
  for (const path of scenario.requiredPayloadPaths ?? []) {
    if (
      events.every((event) => getPathValue(event.payload, path) === undefined)
    ) {
  ...
  const results = scenarios.map((scenario) => {
    const sessions = [...grouped.entries()]
      .filter(([, sessionEvents]) =>
        scenario.scenarioId
          ? sessionEvents.some(
              (event) => event.scenarioId === scenario.scenarioId,
            )
          : true,
      )
