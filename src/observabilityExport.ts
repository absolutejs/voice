...
const collectReplayDeliveryDestinations = (
  ...
  if (Array.isArray(value.destinations)) {
    return value.destinations.filter(
      (
        destination,
      ): destination is VoiceObservabilityExportDeliveryDestinationResult =>
...
export const buildVoiceObservabilityExportReplayReport = (
  ...
  const manifest =
    ...
    (isRecord(records.databaseRecord)
      ? records.databaseRecord.manifest
      : undefined);
  const artifactIndex =
...
export const evaluateVoiceObservabilityExportReplayEvidence = (
  ...
  if (
    report.summary.failedDeliveryDestinations > maxFailedDeliveryDestinations
  ) {
    issues.push(
...
export const renderVoiceObservabilityExportReplayHTML = (
  ...
  const issues =
    report.issues
      ...
      .join("") || '<tr><td colspan="4">No replay issues.</td></tr>';
...
const deliverObservabilityExportToPostgres = async (input: {
  ...
  await sql.unsafe(
    `CREATE SCHEMA IF NOT EXISTS ${quoteObservabilityIdentifier(schema)}`,
  );
  await sql.unsafe(
...
export const buildVoiceObservabilityExport = async (
  ...
  const sessionIds = await time("sessionIds", () =>
    collectSessionIds({
      auditEvents,
      callDebuggerReports,
      events,
  ...
  const operationArtifacts = await time("operationArtifacts", () =>
  ...
  const sessionSnapshotArtifacts = await time("sessionSnapshotArtifacts", () =>
  ...
  const callDebuggerArtifacts = await time("callDebuggerArtifacts", () =>
    callDebuggerReports.map((report) =>
  ...
  const issues = await time("issues", () =>
    collectIssues({
      artifacts,
...
export const deliverVoiceObservabilityExport = async (
  ...
  const destinations = await Promise.all(
    options.destinations.map(async (destination) => {
      const destinationId =
        destination.id ??
        `${destination.kind}-${destination.label ?? "export"}`;
      const label =
        destination.label ??
