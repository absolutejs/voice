...
export const withVoiceIntegrationEventId = <
	TEvent extends Omit<VoiceIntegrationEvent, 'id'> = Omit<VoiceIntegrationEvent, 'id'>
>(
	id: string,
	event: TEvent
): TEvent & { id: string } => ({
...
export const applyVoiceOpsTaskPolicy = (
	...
	const updatedTask = {
		...
		dueAt:
			typeof policy.dueInMs === 'number' ? at + Math.max(0, policy.dueInMs) : task.dueAt,
...
export const resolveVoiceOpsTaskAssignment = (input: {
	...
	task: StoredVoiceOpsTask;
}) => input.rules?.find((rule) => matchesVoiceOpsTaskAssignmentRule(input.task, rule));
...
export const applyVoiceOpsTaskAssignmentRule = (
	...
	return ensureTaskHistory(updatedTask, {
		...
		detail:
			input.detail ??
			rule.description ??
			(rule.name ? `Applied assignment rule ${rule.name}` : 'Applied assignment rule'),
		type: 'assigned'
	});
};
