...
const resolveCallDebuggerSessionId = async (
  ...
  const events =
    options.events ?? (await options.store?.list({ limit: 1000 })) ?? [];
  ...
  return (
    resolved ?? resolveLatestVoiceCallDebuggerSessionId(events) ?? "latest"
  );
};
...
const isVoiceSessionSnapshot = (
  ...
  (value as VoiceSessionSnapshot).schema ===
  "absolute.voice.session.snapshot.v1";
