...
const isStore = <T>(value: unknown): value is { list: () => T[] | Promise<T[]> } =>
	Boolean(value) &&
	typeof value === 'object' &&
	value !== null &&
	'list' in value;

const asArray = async <T>(value: T[] | { list: () => T[] | Promise<T[]> } | undefined) =>
...
export const buildVoicePostCallAnalysisReport = async (
	...
	const sessionId =
		options.sessionId ??
		(reviewId?.endsWith(':review') ? reviewId.slice(0, -':review'.length) : undefined);
	const allTasks = await asArray(options.tasks);
	const tasks = allTasks.filter((task) =>
		reviewId
			? task.reviewId === reviewId || task.intakeId === reviewId || matchesReview(reviewId, task.id)
			: false
	);
