...
const parseTraceTypeFilter = (
  value: unknown,
): VoiceTraceEventType | VoiceTraceEventType[] | undefined => {
...
const filterByDiagnosticsQuery = (
  ...
  return filterVoiceTraceEvents(
    ...
    (event) =>
      ...
      (!status ||
        event.payload.providerStatus === status ||
...
export const buildVoiceDiagnosticsMarkdown = (
  ...
  const trace = renderVoiceTraceMarkdown(events, {
    ...
    title:
      options.title ?? `Voice Diagnostics ${summary.sessionId ?? ""}`.trim(),
  });
...
const renderDiagnosticsIndex = (input: {
  ...
  const rows = [...sessions.entries()]
    .sort(
      ([, left], [, right]) => (right.at(-1)?.at ?? 0) - (left.at(-1)?.at ?? 0),
    )
    .slice(0, 50)
