...
export type VoiceTelephonyOutcomePolicy = {
  ...
  transferTarget?:
    | string
    | ((event: VoiceTelephonyOutcomeProviderEvent) => string | undefined);
...
> = {
  apply?:
    | boolean
    | ((input: VoiceTelephonyWebhookDecision<TResult>) => boolean);
  context?: TContext;
...
export const evaluateVoiceTelephonyWebhookNormalizationEvidence = (
  ...
  const applied = decisions.filter(
    (decision) => decision.applied === true,
  ).length;
  ...
  const replayRejectedVerificationAttempts =
    rejectedVerificationAttempts.filter(
  ...
  if (
    ...
    duplicateCampaignOutcomesApplied > input.maxDuplicateCampaignOutcomesApplied
  ) {
    issues.push(
  ...
  if (
    ...
    rejectedVerificationAttempts.length < input.minRejectedVerificationAttempts
  ) {
    issues.push(
  ...
  if (
    ...
    rejectedVerificationSideEffects > input.maxRejectedVerificationSideEffects
  ) {
    issues.push(
...
const resolveTransferTarget = (
  ...
  return typeof policy.transferTarget === "string" &&
...
export const resolveVoiceTelephonyOutcome = (
  ...
  if (
    answeredBy &&
    normalizeList(policy.machineDetectionVoicemailValues, []).has(answeredBy)
  ) {
  ...
  if (
    target &&
    status &&
    normalizeList(policy.transferStatuses, []).has(status)
  ) {
...
export const createVoiceTelephonyWebhookHandler =
  <
    ...
    TResult = unknown,
  >(
    options: VoiceTelephonyWebhookHandlerOptions<
      TContext,
      TSession,
      TResult
    > = {},
