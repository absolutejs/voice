...
import {
  ...
  type VoiceIncidentRecoveryTrendReport,
  type VoiceIncidentRecoveryTrendSloOptions,
...
export type VoiceProductionReadinessRoutesOptions = {
  ...
  bargeInReports?:
    ...
    | ((input: {
        ...
        | Promise<readonly VoiceBargeInReport[]>
        | readonly VoiceBargeInReport[]);
  carriers?:
    | false
  ...
  incidentRecoveryTrend?:
    ...
    | ((input: {
        ...
        | Promise<VoiceIncidentRecoveryTrendReport>
        | VoiceIncidentRecoveryTrendReport);
  ...
  reconnectContracts?:
    | false
    | readonly VoiceProductionReconnectEvidence[]
    | ((input: {
        query: Record<string, unknown>;
        request: Request;
      }) =>
        | Promise<readonly VoiceProductionReconnectEvidence[]>
        | readonly VoiceProductionReconnectEvidence[]);
  proofSources?:
    | false
...
const isVoiceProfileSwitchReadinessReport = (
  ...
  typeof (value as VoiceProfileSwitchReadinessReport).generatedAt ===
    "string" &&
...
const resolvePhoneAgentSmokes = async (
  ...
  if (
    options.phoneAgentSmokes === false ||
    options.phoneAgentSmokes === undefined
  ) {
    return undefined;
  }
...
const resolveTelephonyMedia = async (
  ...
  if (
    options.telephonyMedia === false ||
    options.telephonyMedia === undefined
  ) {
    return undefined;
  }
...
const resolveReconnectContracts = async (
...
): Promise<VoiceReconnectContractReport[] | undefined> => {
  if (
  ...
  const reports =
    typeof options.reconnectContracts === "function"
...
const resolveBargeInReports = async (
  ...
  if (
    options.bargeInReports === false ||
    options.bargeInReports === undefined
  ) {
    return undefined;
  }
...
const resolveDeliveryRuntime = async (
  ...
  if (
    options.deliveryRuntime === false ||
    options.deliveryRuntime === undefined
  ) {
    return undefined;
  }
...
const resolveAuditRequirement = (
  requirement: VoiceAuditEventType | VoiceProductionReadinessAuditRequirement,
): VoiceProductionReadinessAuditRequirement =>
  ...
  const missing = audit.require.filter((requirement) => {
    ...
    if (requirement.outcomes && requirement.outcomes.length > 0) {
      return !matching.some(
        (event) =>
          event.outcome && requirement.outcomes?.includes(event.outcome),
      );
    }
...
const resolveIncidentRecoveryTrend = async (
  options: VoiceProductionReadinessRoutesOptions,
  input: { query: Record<string, unknown>; request: Request },
): Promise<VoiceIncidentRecoveryTrendReport | undefined> => {
...
const summarizeDeliveryRuntime = (
  ...
  const deadLettered = queues.reduce(
    (sum, queue) => sum + queue.deadLettered,
    0,
  );
...
const buildOperationsRecordLinks = (input: {
  ...
  const providerErrors = input.events
    ...
    .map(
      (event): VoiceProductionReadinessOperationsRecordLink => ({
    ...
    input.telephonyMedia && input.telephonyMedia.status !== "pass"
      ? input.telephonyMedia.carriers
          ...
          .flatMap((carrier) => {
            ...
            const ids =
              sessionIds.length > 0 ? sessionIds : [fallbackSessionId];
...
const applyProfileReadinessDefaults = (
  options: VoiceProductionReadinessRoutesOptions,
): VoiceProductionReadinessRoutesOptions => {
  ...
  if (resolvedOptions.browserMediaMinActiveCandidatePairs === undefined) {
    resolvedOptions.browserMediaMinActiveCandidatePairs =
      defaults.browserMediaMinActiveCandidatePairs;
  }
  if (resolvedOptions.browserMediaMinLiveAudioTracks === undefined) {
    resolvedOptions.browserMediaMinLiveAudioTracks =
      defaults.browserMediaMinLiveAudioTracks;
  }
  if (resolvedOptions.browserMediaMinTotalStats === undefined) {
    resolvedOptions.browserMediaMinTotalStats =
      defaults.browserMediaMinTotalStats;
  }
  if (resolvedOptions.reconnectContractsMinCount === undefined) {
    resolvedOptions.reconnectContractsMinCount =
      defaults.reconnectContractsMinCount;
  }
  if (resolvedOptions.reconnectContractsMinSnapshotsPerContract === undefined) {
    resolvedOptions.reconnectContractsMinSnapshotsPerContract =
      defaults.reconnectContractsMinSnapshotsPerContract;
  }
  if (
    "telephonyMediaMinCarriers" in defaults &&
    resolvedOptions.telephonyMediaMinCarriers === undefined
  ) {
    resolvedOptions.telephonyMediaMinCarriers =
      defaults.telephonyMediaMinCarriers;
  }
  if (
    "telephonyMediaMinMediaEvents" in defaults &&
    resolvedOptions.telephonyMediaMinMediaEvents === undefined
  ) {
    resolvedOptions.telephonyMediaMinMediaEvents =
      defaults.telephonyMediaMinMediaEvents;
  }
...
export const buildVoiceProductionReadinessReport = async (
  ...
  const providerRecovery =
    summarizeVoiceProviderFallbackRecovery(readinessEvents);
  const [
    quality,
    providers,
    sessions,
    ...
    time("incidentRecoveryTrend", () =>
      resolveIncidentRecoveryTrend(options, { query, request }),
    ),
  ...
  const incidentRecoveryOutcomeReadiness =
    incidentRecoveryOutcomes &&
    options.incidentRecoveryOutcomeReadiness !== false
      ? buildVoiceIncidentRecoveryOutcomeReadinessCheck(
          incidentRecoveryOutcomes,
          {
            href:
      ...
      : undefined;
  const incidentRecoveryTrendReadiness =
    incidentRecoveryTrend && options.incidentRecoveryTrendSLO !== false
      ? buildVoiceIncidentRecoveryTrendSLOReadinessCheck(
          incidentRecoveryTrend,
          {
            href:
              options.incidentRecoveryTrendSLO?.href ??
            ...
            maxRegressionRate:
              options.incidentRecoveryTrendSLO?.maxRegressionRate,
            maxUnchangedRate:
              options.incidentRecoveryTrendSLO?.maxUnchangedRate,
            minActions: options.incidentRecoveryTrendSLO?.minActions,
            minCycles: options.incidentRecoveryTrendSLO?.minCycles,
            minImprovementRate:
              options.incidentRecoveryTrendSLO?.minImprovementRate,
  ...
  const checks: VoiceProductionReadinessCheck[] = [
      ...
      actions:
        quality.status === "pass"
          ...
          : [
              {
                description:
                  "Open the quality report to inspect failing gates.",
                href: options.links?.quality ?? "/quality",
    ...
    ...(incidentRecoveryTrendReadiness ? [incidentRecoveryTrendReadiness] : []),
    {
      ...
      status:
        failedSessions > 0 ? "fail" : sessions.length === 0 ? "warn" : "pass",
        ...
        failedSessions > 0
          ...
          : sessions.length === 0
            ? [
                {
                  description:
                    "Open sessions after running a smoke or live call.",
  ...
  const mediaPipelineSummary = mediaPipeline
    ? ({
      ...
      >)
    : undefined;
  const browserMediaBreadthFailures: string[] = browserMedia
    ? [
        options.browserMediaMinActiveCandidatePairs !== undefined &&
        typeof options.browserMediaMinActiveCandidatePairs === "number" &&
        options.browserMediaMinActiveCandidatePairs >= 0 &&
        browserMedia.activeCandidatePairs <
          options.browserMediaMinActiveCandidatePairs
          ? `Expected at least ${options.browserMediaMinActiveCandidatePairs} active candidate pair(s), observed ${browserMedia.activeCandidatePairs}.`
          : undefined,
        options.browserMediaMinLiveAudioTracks !== undefined &&
        typeof options.browserMediaMinLiveAudioTracks === "number" &&
        options.browserMediaMinLiveAudioTracks >= 0 &&
        browserMedia.liveAudioTracks < options.browserMediaMinLiveAudioTracks
          ? `Expected at least ${options.browserMediaMinLiveAudioTracks} live audio track(s), observed ${browserMedia.liveAudioTracks}.`
          : undefined,
        options.browserMediaMinTotalStats !== undefined &&
        typeof options.browserMediaMinTotalStats === "number" &&
        options.browserMediaMinTotalStats >= 0 &&
        browserMedia.totalStats < options.browserMediaMinTotalStats
          ? `Expected at least ${options.browserMediaMinTotalStats} WebRTC stats sample(s), observed ${browserMedia.totalStats}.`
          : undefined,
      ].filter(
  ...
  const browserMediaSummary = browserMedia
    ? ({
        ...
        status:
          browserMedia.status === "pass" &&
          browserMediaBreadthFailures.length === 0
            ? "pass"
            : "fail",
      } satisfies NonNullable<
      ...
      >)
    : undefined;
  let telephonyMediaBreadthFailures: string[] = [];
  const telephonyMediaSummary = telephonyMedia
    ? (() => {
        const telephonyMediaTotalMediaEvents = telephonyMedia.carriers.reduce(
          (total, carrier) => total + carrier.lifecycle.mediaEvents,
          0,
        );
        const failures: string[] = [
          options.telephonyMediaMinCarriers !== undefined &&
          typeof options.telephonyMediaMinCarriers === "number" &&
          options.telephonyMediaMinCarriers >= 0 &&
          telephonyMedia.carriers.length < options.telephonyMediaMinCarriers
            ? `Expected at least ${options.telephonyMediaMinCarriers} telephony carrier(s), observed ${telephonyMedia.carriers.length}.`
            : undefined,
          options.telephonyMediaMinMediaEvents !== undefined &&
          typeof options.telephonyMediaMinMediaEvents === "number" &&
          options.telephonyMediaMinMediaEvents >= 0 &&
          telephonyMediaTotalMediaEvents < options.telephonyMediaMinMediaEvents
            ? `Expected at least ${options.telephonyMediaMinMediaEvents} telephony media event(s), observed ${telephonyMediaTotalMediaEvents}.`
            : undefined,
        ].filter(
          (message): message is string =>
            typeof message === "string" && message.length > 0,
        ...
        return {
          ...
          status:
            telephonyMedia.status === "pass" &&
            telephonyMediaBreadthFailures.length === 0
              ? "pass"
              : "fail",
        } satisfies NonNullable<
    ...
    : undefined;
  const browserMediaBreadthSummary =
    browserMediaBreadthFailures.length > 0
      ? browserMediaBreadthFailures
      : undefined;
  const telephonyMediaBreadthSummary =
    telephonyMediaBreadthFailures.length > 0
      ? telephonyMediaBreadthFailures
      : undefined;
  checks.push({
    detail:
      liveLatency.total === 0
        ...
        mediaPipelineSummary.status === "pass"
          ...
          : {
              evidenceHref:
                ...
                "/voice/media-pipeline",
              observed:
                firstIssue?.code ?? `${mediaPipelineSummary.issues} issue(s)`,
              remediation:
    ...
    checks.push({
      detail:
        browserMediaSummary.status === "pass"
          ...
          : browserMediaBreadthSummary
            ? browserMediaBreadthSummary.join("; ")
            : (firstIssue?.message ??
      ...
      gateExplanation:
        browserMediaSummary.status === "pass"
          ...
          : browserMediaBreadthSummary
            ? {
                evidenceHref:
                  options.links?.browserMedia ?? "/voice/browser-media",
                observed: browserMediaBreadthSummary.join("; "),
                remediation:
                  "Collect richer browser media evidence and rerun readiness so active candidate-pair, track, and stats thresholds are met.",
            ...
            : {
                evidenceHref:
                  options.links?.browserMedia ?? "/voice/browser-media",
                observed:
                  firstIssue?.code ?? `${browserMediaSummary.issues} issue(s)`,
                remediation:
    ...
    checks.push({
      detail:
        telephonyMediaSummary.status === "pass"
          ...
          : telephonyMediaBreadthSummary
            ? telephonyMediaBreadthSummary.join("; ")
            : (firstIssue ??
        ...
        telephonyMediaSummary.status === "pass"
          ...
          : telephonyMediaBreadthSummary
            ? {
                ...
                remediation:
                  "Collect richer telephony lifecycle evidence and rerun readiness so carrier/media-event thresholds are met.",
            ...
            : {
                evidenceHref:
                  ...
                  "/voice/telephony-media",
                observed:
                  firstIssue ?? `${telephonyMediaSummary.issues} issue(s)`,
                remediation:
                ...
                thresholdLabel:
                  "Telephony media serializer and lifecycle status",
                unit: "status",
              },
  ...
  const providerRoutingContractSummary = providerRoutingContracts
    ? ({
        failed: providerRoutingContracts.filter((report) => !report.pass)
          .length,
  ...
  const telephonyWebhookSecuritySummary = telephonyWebhookSecurity
    ? ({
      ...
      >)
    : undefined;
  let reconnectContractBreadthFailures: string[] = [];
  const reconnectContractSummary = reconnectContracts
    ? (() => {
        const reconnectContractCountMin =
          typeof options.reconnectContractsMinCount === "number" &&
          options.reconnectContractsMinCount >= 0
            ? options.reconnectContractsMinCount
            : undefined;
        const reconnectContractSnapshotsMin =
          typeof options.reconnectContractsMinSnapshotsPerContract ===
            "number" && options.reconnectContractsMinSnapshotsPerContract >= 0
            ? options.reconnectContractsMinSnapshotsPerContract
            : undefined;
        const reconnectSnapshotShortfallCount =
          reconnectContractSnapshotsMin === undefined
            ? 0
            : reconnectContracts.filter(
                (report) =>
                  typeof report.snapshotCount === "number" &&
                  report.snapshotCount < reconnectContractSnapshotsMin,
              ).length;
        const reconnectContractCountShortfall =
          reconnectContractCountMin === undefined
            ? 0
            : Math.max(
                0,
                reconnectContractCountMin - reconnectContracts.length,
              );
        reconnectContractBreadthFailures =
          reconnectContractCountShortfall > 0
            ? [
        ...
        if (reconnectSnapshotShortfallCount > 0) {
          reconnectContractBreadthFailures.push(
            `${reconnectSnapshotShortfallCount} reconnect contract(s) with fewer than ${reconnectContractSnapshotsMin} snapshot(s).`,
          );
        }
        const failedReports = reconnectContracts.filter(
          (report) =>
            !report.pass ||
            !report.summary.reconnected ||
        ...
        const resumeLatencies = reconnectContracts
          ...
          .filter((value): value is number => typeof value === "number");
        const reconnectContractBreadthFailureCount =
          reconnectContractBreadthFailures.length > 0
            ? Math.min(
                reconnectContracts.length,
                reconnectContractBreadthFailures.length,
              )
            : 0;
        const passedCount = Math.max(
          0,
          reconnectContracts.length -
            (failedReports.length + reconnectContractBreadthFailureCount),
        );
    ...
    : undefined;
  const reconnectContractBreadthSummary =
    reconnectContractBreadthFailures.length > 0
      ? reconnectContractBreadthFailures
      : undefined;
  ...
  const incidentRecoveryOutcomeSummary =
    incidentRecoveryOutcomes && incidentRecoveryOutcomeReadiness
      ? ({
        ...
        >)
      : undefined;
  const incidentRecoveryTrendSummary =
    incidentRecoveryTrend && incidentRecoveryTrendReadiness
      ? ({
          ...
          VoiceProductionReadinessReport["summary"]["incidentRecoveryTrend"]
        >)
      : undefined;
          ...
          : [
              {
                ...
                href:
                  options.links?.agentSquadContracts ?? "/agent-squad-contract",
  ...
  if (providerRoutingContractSummary) {
    checks.push({
      ...
      proofSource: proofSource("providerRoutingContracts", "providerRouting"),
    ...
    checks.push({
      ...
      gateExplanation:
        providerSloSummary.status === "pass"
          ? undefined
          : calibratedGateExplanation({
              evidenceHref: firstIssue?.sessionId
                ? voiceOperationsRecordHref(
                    options.links?.operationsRecords ?? "/voice-operations",
                    firstIssue.sessionId,
                  )
    ...
    checks.push({
      detail:
        reconnectContractSummary.status === "pass"
          ...
          : reconnectContractBreadthSummary &&
              reconnectContractBreadthSummary.length > 0
            ? reconnectContractBreadthSummary.join("; ")
            : reconnectContractSummary.total === 0
              ...
              : options.reconnectResumeFailAfterMs !== undefined &&
                ...
                : `${reconnectContractSummary.failed} reconnect contract(s) failed or did not observe reconnect resume.`,
      href:
        ...
        reconnectContractSummary.status === "pass"
          ...
          : reconnectContractBreadthSummary &&
            ...
            ? {
                ...
                remediation:
                  "Increase reconnect contract breadth (count and snapshot depth) before marking this gate as passing.",
      ...
      gateExplanation:
        bargeInSummary.status === "pass"
          ...
          : calibratedGateExplanation({
              ...
              unit:
                typeof bargeInReports?.[0]?.thresholdMs === "number"
                  ? "ms"
                  : "count",
            }),
  ...
  if (profileSwitchReadiness) {
    const issueLabels = profileSwitchReadiness.issues.map(
      (issue) => issue.code,
    );
    checks.push({
      detail:
  ...
  if (auditDeliveries) {
    checks.push({
      ...
      href: options.links?.auditDeliveries ?? options.links?.audit ?? "/audit",
