...
const isDecisionTrace = (event: unknown): event is VoiceProviderDecisionTrace =>
  Boolean(
    event &&
    typeof event === "object" &&
    "provider" in event &&
...
export const buildVoiceProviderDecisionTraceReport = async (
  ...
  const decisions = listVoiceProviderDecisionTraces(rawEvents).filter(
    (decision) => {
      ...
      if (
        ...
        now - decision.at > options.maxAgeMs
      ) {
        return false;
      }
      return true;
  ...
  const surfaceReports = [...surfaces.entries()]
    ...
    .map(([surface, surfaceDecisions]) => {
      ...
      return {
        ...
        errors: surfaceDecisions.filter(
        ...
        reasons: uniqueSorted(
          surfaceDecisions.map((decision) => decision.reason),
        ),
  ...
  return {
    ...
    summary: {
      ...
      errors: decisions.filter((decision) => decision.status === "error")
        .length,
