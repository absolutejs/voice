...
> = {
  backoffMs?: number;
  body?: (input: { events: StoredVoiceTraceEvent[] }) => Promise<TBody> | TBody;
  eventTypes?: VoiceTraceEventType[];
  fetch?: typeof fetch;
...
export type VoiceTraceRedactionReplacement =
  | string
  | ((input: { key?: string; path: string[]; value: string }) => string);
...
export const createVoiceTraceSinkDeliveryRecord = (
  input: {
    ...
    Omit<VoiceTraceSinkDeliveryRecord, "createdAt" | "events" | "id">
  >,
...
const matchesTraceFilter = (
  ...
  if (
    ...
    event.scenarioId !== filter.scenarioId
  ) {
    return false;
  }
...
>(
  ...
  let candidates = filterVoiceTraceEvents(events, options.filter).filter(
    (event) => isPruneTimeMatch(event, options),
  );
  ...
  deliver: async ({ events }) => {
    ...
    if (typeof fetchImpl !== "function") {
      return {
        ...
        error:
          "Trace sink delivery failed: fetch is not available in this runtime.",
...
export const exportVoiceTrace = async (input: {
  ...
  return {
    exportedAt: Date.now(),
    events: input.redact
      ? redactVoiceTraceEvents(events, input.redact)
      : events,
    filter: input.filter,
...
const renderTraceEventMarkdown = (
  ...
  const offset =
    startedAt === undefined
      ? `${event.at}`
      : `+${Math.max(0, event.at - startedAt)}ms`;
