...
export const createVoiceAudioPlayer = (
  ...
  const lookaheadSeconds =
    (options.lookaheadMs ?? DEFAULT_LOOKAHEAD_MS) / 1_000;
  ...
  const ensureAudioContext = async () => {
    ...
    if (options.createAudioContext) {
      audioContext =
        options.createAudioContext() as unknown as MinimalAudioContext;
    } else {
  ...
  const player: VoiceAudioPlayer = {
    ...
    interrupt: async () => {
      ...
      const playbackStopLatencyMs =
        Date.now() - startedAt + estimateOutputStopLatencyMs(context);
      setState({
        isActive: false,
...
export const decodeVoiceAudioChunk = (
  audioContext: Pick<MinimalAudioContext, "createBuffer">,
  chunk: {
    chunk: Uint8Array;
    format: AudioFormat;
  },
  ...
  decodePCM16LEChunk(
    audioContext as MinimalAudioContext,
    chunk as VoiceAudioChunk,
  );
