...
const formatKind = (kind: VoiceProviderCapabilityKind) => kind.toUpperCase();
  ...
  const capabilities = model.capabilities.length
    ? `<div class="absolute-voice-provider-capabilities__providers">${model.capabilities
        .map(
          (
            capability,
          ) => `<article class="absolute-voice-provider-capabilities__provider absolute-voice-provider-capabilities__provider--${escapeHtml(capability.status)}">
  <header>
