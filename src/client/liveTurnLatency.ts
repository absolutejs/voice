...
export const createVoiceLiveTurnLatencyMonitor = (
  ...
  const reportCompletedEvent = async (event: VoiceLiveTurnLatencyEvent) => {
    ...
    if (!response.ok) {
      throw new Error(
        `Voice live turn latency report failed: HTTP ${response.status}`,
      );
    }
  };
  const completePending = (
    input: Pick<
      ...
      "assistantAudioAt" | "assistantTextAt"
    >,
  ) => {
    if (!pending) {
      return;
    ...
    const completedAt =
      input.assistantAudioAt ?? input.assistantTextAt ?? clock();
    const latencyMs = Math.max(0, completedAt - pending.startedAt);
    const status =
      latencyMs > failAfterMs
        ? "fail"
        : latencyMs > warnAfterMs
          ? "warn"
          : "pass";
    pending = {
      ...pending,
