...
export type VoiceProviderStatusCardView<TProvider extends string = string> =
...
export type VoiceProviderStatusWidgetOptions =
  ...
  const providers = model.providers.length
    ? `<div class="absolute-voice-provider-status__providers">${model.providers
        .map(
          (
            provider,
          ) => `<article class="absolute-voice-provider-status__provider absolute-voice-provider-status__provider--${escapeHtml(provider.status)}">
  <header>
