...
export const createVoiceOpsStatusViewModel = (
  ...
  const surfaces = Object.entries(report?.surfaces ?? {}).map(
    ([id, surface]) => {
      const failed =
        readNumber(surface, "failed") || readNumber(surface, "degraded");
      const total = readNumber(surface, "total");
      const status =
  ...
  const surfaces = model.surfaces.length
    ? model.surfaces
        .map(
          (
            surface,
          ) => `<li class="absolute-voice-ops-status__surface absolute-voice-ops-status__surface--${escapeHtml(surface.status)}">
