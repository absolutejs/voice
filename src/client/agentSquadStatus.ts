...
export type VoiceAgentSquadStatusClientOptions =
...
const getPayloadString = (event: VoiceTraceTimelineEvent, key: string) =>
  getString(
    (event as VoiceTraceTimelineEvent & { payload?: Record<string, unknown> })
      .payload?.[key],
  );
...
const deriveSessionSpecialist = (
  ...
  const handoffEvents = events.filter(
    (event) => event.type === "agent.handoff",
  );
  ...
  const currentTarget =
    handoffStatus === "blocked" || handoffStatus === "unknown-target"
      ...
      : (getPayloadString(lastHandoff ?? latest, "targetAgentId") ??
        latestAgentId);
...
export const buildVoiceAgentSquadStatusReport = (
  ...
  const sessions = (timeline?.sessions ?? [])
    .filter(
      (session) =>
        !options.sessionId || session.sessionId === options.sessionId,
    )
