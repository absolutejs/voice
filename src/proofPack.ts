...
const createSnapshotTraceDeliveryStore = (
  ...
  remove: async () => {
    throw new Error(
      "Voice proof refresh snapshot delivery store is read-only.",
    );
  },
  set: async () => {
    throw new Error(
  ...
  const [productionReadiness, providerSlo, supportBundle, observabilityExport] =
    await Promise.all([
      ...
      options.loadProviderSlo
        ? context.time("providerSlo", () =>
            options.loadProviderSlo?.(loaderInput),
          )
        : undefined,
...
export const createVoiceProofPackOperationsRecordSection = (
  ...
  return {
    ...
    status:
      failed > 0 || errors > 0
        ? "fail"
        : warnings > 0 || fallbacks > 0
          ? "warn"
          : "pass",
    summary:
      "Per-call operations records, trace errors, and provider recovery.",
...
const buildDerivedProofPackSections = (
  ...
  ...(input.observabilityExport
    ? buildVoiceProofPackFromObservabilityExport(input.observabilityExport)
        .sections
    : []),
];
...
export const createVoiceProofPackRoutes = (
  ...
  if (jsonPath !== false) {
    app.get(
      ...
      async () =>
        new Response(
          JSON.stringify(await resolveProofPack(options.source), null, 2),
          {
            headers: {
