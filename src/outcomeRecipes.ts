...
> = Pick<
	...
	'createTaskFromReview' | 'resolveTaskPolicy' | 'taskAssignmentRules' | 'taskPolicies'
> & {
	description: string;
...
>(
	...
	const taskPolicies: VoiceOpsDispositionTaskPolicies = {
		...
		escalated: {
			...
			dueInMs: Math.min(options.dueInMs ?? defaults.defaultDueInMs, 10 * 60_000),
		...
		transferred: {
			assignee: options.assignee,
			dueInMs: Math.min(options.dueInMs ?? defaults.defaultDueInMs, 20 * 60_000),
			name: `${name}-transfer-check`,
			priority: options.priority ?? defaults.defaultPriority,
			queue: name === 'warm-transfer'
	...
	].filter((rule) => rule.assign || rule.queue) satisfies VoiceOpsTaskAssignmentRules;
