...
const DEFAULT_LINKS: VoiceOpsConsoleLink[] = [
  {
    description:
      "Quality gates for CI, deploy checks, and production readiness.",
    href: "/quality",
    label: "Quality",
...
export const buildVoiceOpsConsoleReport = async (
  ...
  return {
    ...
    sessions: {
      ...
      healthy: sessions.filter((session) => session.status === "healthy")
        .length,
...
<p><a href="${escapeHtml(link.href)}">Open ${escapeHtml(link.label)}</a>${link.statusHref ? ` · <a href="${escapeHtml(link.statusHref)}">Status</a>` : ""}</p>
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>${escapeHtml(title)}</title><style>body{font-family:ui-sans-serif,system-ui,sans-serif;background:#101316;color:#f6f2e8;margin:0}main{max-width:1180px;margin:auto;padding:32px}a{color:#fbbf24}header{display:flex;justify-content:space-between;gap:24px;align-items:flex-start;margin-bottom:24px}.eyebrow{color:#fbbf24;font-weight:800;letter-spacing:.08em;text-transform:uppercase}h1{font-size:clamp(2.2rem,5vw,4.5rem);line-height:.95;margin:.2rem 0 1rem}.muted{color:#a8b0b8}.grid{display:grid;gap:14px;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));margin:20px 0}.metric,.surface{background:#181d22;border:1px solid #2a323a;border-radius:20px;padding:18px}.metric strong{display:block;font-size:2.2rem;margin:.25rem 0}.pass,.healthy{color:#86efac}.fail,.failed,.degraded{color:#fca5a5}.surfaces{display:grid;gap:16px;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));margin:24px 0}table{width:100%;border-collapse:collapse;background:#181d22;border-radius:16px;overflow:hidden;margin:12px 0 28px}td,th{border-bottom:1px solid #2a323a;padding:12px;text-align:left}section{margin-top:30px}@media(max-width:700px){main{padding:20px}header{display:block}}</style></head><body><main><header><div><p class="eyebrow">Self-hosted voice operations</p><h1>${escapeHtml(title)}</h1><p class="muted">One deployable control plane for quality gates, failover, traces, sessions, handoffs, and provider health.</p></div><p class="muted">Checked ${escapeHtml(new Date(report.checkedAt).toLocaleString())}</p></header><div class="grid">${renderMetricCard({ href: "/quality", label: "Quality", status: report.quality.status, value: report.quality.status })}${renderMetricCard({ href: "/diagnostics", label: "Events", value: report.eventCount })}${renderMetricCard({ href: "/sessions", label: "Sessions", status: report.sessions.failed > 0 ? "failed" : "healthy", value: report.sessions.total })}${renderMetricCard({ href: "/handoffs", label: "Handoffs failed", status: report.handoffs.failed > 0 ? "failed" : "healthy", value: report.handoffs.failed })}${renderMetricCard({ href: "/resilience", label: "Providers degraded", status: report.providers.degraded > 0 ? "degraded" : "healthy", value: report.providers.degraded })}</div><section><h2>Operational Surfaces</h2><div class="surfaces">${links}</div></section><section><h2>Recent Sessions</h2><table><thead><tr><th>Session</th><th>Status</th><th>Turns</th><th>Errors</th><th>Replay</th></tr></thead><tbody>${sessions}</tbody></table></section><section><h2>Recent Provider Routing</h2><table><thead><tr><th>Kind</th><th>Provider</th><th>Status</th><th>Elapsed</th><th>Session</th></tr></thead><tbody>${routing}</tbody></table></section></main></body></html>`;
};
