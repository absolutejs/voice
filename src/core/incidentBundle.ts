...
const retentionTimeMatch = (
  ...
  if (
    ...
    artifact.createdAt >= options.before
  ) {
    return false;
  }
  if (
...
export const pruneVoiceIncidentBundleArtifacts = async (
  ...
  if (!dryRun) {
    await Promise.all(
      selected.map((artifact) => options.store.remove(artifact.id)),
    );
  }
...
const renderIncidentMarkdown = (input: {
  ...
  const lines = [
    ...
    ...(input.record.handoffs.length
      ? input.record.handoffs.map((handoff) =>
