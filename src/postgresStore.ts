import { withVoiceIntegrationEventId, withVoiceOpsTaskId } from "./ops";
...
const resolveQualifiedTableName = (input: {
  ...
  const schema = normalizeIdentifierSegment(
    input.options.schemaName ?? "public",
  );
...
const createPostgresAuditEventStoreWithClient = <
...
const createPostgresAuditSinkDeliveryStoreWithClient = <
...
>(
  ...
  createPostgresReviewStoreWithClient(
    createVoicePostgresClient(options),
    options,
  );
...
> => {
  ...
  return {
    audit: createPostgresAuditEventStoreWithClient<TAudit>(client, options),
    auditDeliveries:
      createPostgresAuditSinkDeliveryStoreWithClient<TAuditDelivery>(
        client,
        options,
      ),
    ...
    traceDeliveries:
      createPostgresTraceSinkDeliveryStoreWithClient<TTraceDelivery>(
        client,
        options,
      ),
