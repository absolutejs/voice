...
export const renderVoiceAuditTrailHTML = (
  ...
  const chips = report.summary.byType
    .map(
      ([type, count]) =>
        `<span>${escapeHtml(type)} <strong>${count}</strong></span>`,
    )
    .join("");
...
export const createVoiceAuditTrailRoutes = (
  ...
  const exportHtmlPath =
    options.exportHtmlPath ??
    (htmlPath === false ? false : `${htmlPath}/export`);
  ...
  if (htmlPath !== false) {
    routes.get(htmlPath, async ({ query }) => {
      ...
      const body = await (
        options.render ??
        ((value) => renderVoiceAuditTrailHTML(value, { title: options.title }))
      )(report);
