...
> =
  ...
  | {
      ...
      maxToolRounds?: never;
      model?: never;
      selectAgent?: VoiceAgentSquadOptions<
        TContext,
        TSession,
        TResult
      >["selectAgent"];
...
const mergeOps = <TContext, TSession extends VoiceSessionRecord, TResult>(
...
export const summarizeVoiceAssistantRuns = async (
  ...
  const assistantRuns = events.filter(
    (event) => event.type === "assistant.run",
  );
  ...
  for (const event of events.filter(
    (event) => event.type === "assistant.memory",
  )) {
    const assistantId =
      typeof event.payload.assistantId === "string"
        ? event.payload.assistantId
