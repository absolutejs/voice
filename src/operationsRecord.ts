...
const toGuardrailDecision = (
  ...
  findings: Array.isArray(event.payload.findings)
    ? event.payload.findings
        ...
        .filter(
          (finding): finding is VoiceOperationsRecordGuardrailFinding =>
            finding !== undefined,
        )
    : [],
  metadata:
...
const summarizeTelephonyMedia = (
  ...
  const mediaEvents = events
    ...
    .filter(
      (event): event is VoiceOperationsRecordTelephonyMediaEvent =>
        event !== undefined,
    );
  const eventNames = mediaEvents.map((event) => event.event.toLowerCase());

  return {
    audioBytes: mediaEvents.reduce(
    ...
    events: mediaEvents,
    inbound: mediaEvents.filter((event) => event.direction === "inbound")
      .length,
    marks: eventNames.filter((event) => event === "mark").length,
    media: eventNames.filter((event) => event === "media").length,
    outbound: mediaEvents.filter((event) => event.direction === "outbound")
      .length,
...
const toProviderDecision = (
  ...
  const status =
    getString(event.payload.providerStatus) ?? getString(event.payload.status);
...
const resolveOutcome = (
  ...
  return {
    ...
    transferred: agentResults.some(
      (event) => event.payload.transferred === true,
    ),
...
export const buildVoiceOperationsRecord = async (
  ...
  const rawAuditEvents = options.audit
    ? filterVoiceAuditEvents(
        await options.audit.list({ sessionId: options.sessionId }),
      )
    : undefined;
  const auditEvents =
  ...
  const sinkDeliveries =
    integrationEvents?.reduce(
      (total, event) => total + Object.keys(event.sinkDeliveries ?? {}).length,
      0,
    ) ?? 0;
...
export const evaluateVoiceOperationsRecordGuardrails = (
  ...
  const toolNames = uniqueSorted(
    decisions.map((decision) => decision.toolName),
  );
  ...
  if (
    input.minWarned !== undefined &&
    record.guardrails.warned < input.minWarned
  ) {
    issues.push(
  ...
  if (
    input.minPassed !== undefined &&
    record.guardrails.passed < input.minPassed
  ) {
    issues.push(
...
export const evaluateVoiceOperationsRecordProviderRecovery = (
  ...
  if (
    input.minFallbacks !== undefined &&
    summary.fallbacks < input.minFallbacks
  ) {
    issues.push(
...
export const renderVoiceFailureReplayMarkdown = (
  report:
    | Omit<VoiceFailureReplayReport, "incidentMarkdown">
    | VoiceFailureReplayReport,
) => {
        ...
        .map((step) => {
          ...
          const parts = [
            ...
            step.selectedProvider
              ? `selected=${step.selectedProvider}`
              : undefined,
            step.fallbackProvider
              ? `fallback=${step.fallbackProvider}`
              : undefined,
            step.elapsedMs !== undefined
              ? `elapsed=${step.elapsedMs}ms`
              : undefined,
            step.reason,
