...
export type VoiceLiveOpsControlStore = {
	get: (
		sessionId: string
	) => Promise<VoiceLiveOpsControlState | undefined> | VoiceLiveOpsControlState | undefined;
	set: (
		sessionId: string,
...
export const evaluateVoiceLiveOpsControlEvidence = (
	...
	const actions = uniqueSorted(
		results
			...
			.filter((action): action is VoiceLiveOpsAction => isVoiceLiveOpsAction(action))
	);
	const statuses = uniqueSorted(
		controls.map((control) => control.status)
	);
...
export const createVoiceMemoryLiveOpsControlStore = (): VoiceLiveOpsControlStore => {
...
export const buildVoiceLiveOpsControlState = (
	...
	handoffTarget:
		input.action === 'force-handoff' ? input.tag : input.previous?.handoffTarget,
	injectedInstruction:
