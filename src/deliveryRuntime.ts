		...
		summarize: async () => {
			...
			if (config.audit) {
				summary.audit = await summarizeVoiceAuditSinkDeliveries(
					...
					{
						deadLetters: config.audit
							.deadLetters as VoiceAuditSinkDeliveryStore<VoiceAuditSinkDeliveryRecord> | undefined
					}
				);
			}
			if (config.trace) {
				summary.trace = await summarizeVoiceTraceSinkDeliveries(
					...
					{
						deadLetters: config.trace
							.deadLetters as VoiceTraceSinkDeliveryStore<VoiceTraceSinkDeliveryRecord> | undefined
					}
				);
			}
