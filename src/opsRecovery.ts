...
const rollupStatus = (issues: VoiceOpsRecoveryIssue[]): VoiceOpsRecoveryStatus =>
...
>(
	...
	const events =
		options.events ?? (await options.traces?.list({ limit: Math.max(limit, 500) })) ?? [];
	const providers = await summarizeVoiceProviderHealth<TProvider>({
		events,
		providers: options.providers
	});
	const auditDeliveries = options.auditDeliveries
		? await summarizeVoiceAuditSinkDeliveries(await options.auditDeliveries.list(), {
				deadLetters: options.auditDeliveryDeadLetters
			})
		: undefined;
	const traceDeliveries = options.traceDeliveries
		? await summarizeVoiceTraceSinkDeliveries(await options.traceDeliveries.list(), {
