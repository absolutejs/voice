...
export type VoiceProfileSwitchGuardOptions =
...
export const applyVoiceProfileSwitchGuard = async (
  ...
  const isRecommendedBlocked = recommendedProfileId
  ...
  const selectedProfileId =
    action === "switch"
      ? recommendedProfileId
      : (previousProfileId ?? recommendedProfileId);
  const reason =
    action === "disabled"
...
const createDefaultPolicyProofCases = (input: {
  ...
  {
    description:
      "Recommend mode records the recommendation without changing selection.",
...
const auditEventToLiveDecision = (
  ...
  return {
    action:
      ...
      event.action.replace(/^profile\.switch\./, ""),
    at: event.at,
...
export const buildVoiceProfileSwitchLiveDecisionReport = async (
  ...
  const decisions = [
    ...auditEvents
      ...
      .filter((decision): decision is VoiceProfileSwitchLiveDecisionEvidence =>
        Boolean(decision),
      ),
    ...traceEvents
  ...
  const sessions = Array.from(
    ...
    lastDecision: session.decisions.sort(
...
export const buildVoiceProfileSwitchReadinessReport = async (
  ...
  const requirePolicyProof =
    options.requirePolicyProof ?? Boolean(options.policyProof);
