...
export type VoiceHelpdeskTicketSinkOptions =
	VoiceIntegrationHTTPSinkOptions & {
		project?: string;
	};

export type VoiceCRMActivitySinkOptions =
	VoiceIntegrationHTTPSinkOptions & {
		pipeline?: string;
	};
		...
		try {
			...
			if (input.config.signingSecret) {
				...
				headers['x-absolutejs-signature'] =
					await signVoiceIntegrationSinkBody({
						body: input.body,
...
const aggregateVoiceIntegrationDeliveryStatus = (
	...
	const statuses = [
		...(event.deliveryStatus && event.deliveryStatus !== 'pending'
			...
			: []),
		...Object.values(event.sinkDeliveries ?? {}).map((delivery) => delivery.status)
	];
...
const mergeVoiceSinkAggregateStatus = (
	...
	if (event.deliveryStatus === undefined || event.deliveryStatus === 'pending') {
...
const resolveVoiceSinkPriority = (
	...
	typeof event.payload.priority === 'string' ? event.payload.priority : undefined;
...
export const createVoiceZendeskTicketUpdateSink = (
	...
	return {
		deliver: async ({ event }) => {
			const ticketId = await resolveVoiceSinkValue(options.ticketId, event, [
				'zendeskTicketId',
				'ticketId',
				'externalTicketId'
...
export const createVoiceHubSpotTaskUpdateSink = (
	...
	return {
		deliver: async ({ event }) => {
			const taskId = await resolveVoiceSinkValue(options.taskId, event, [
				'hubspotTaskId',
				'hubSpotTaskId',
				'externalTaskId'
			]) ?? await findVoiceExternalObjectId({
...
export const createVoiceLinearIssueSink = (
	...
	const authHeader =
		options.authMode === 'api-key' ? token : `Bearer ${token}`;
...
export const createVoiceLinearIssueUpdateSink = (
	...
	return {
		deliver: async ({ event }) => {
			const issueId = await resolveVoiceSinkValue(options.issueId, event, [
				'linearIssueId',
				'issueId',
				'externalIssueId'
			]) ?? await findVoiceExternalObjectId({
