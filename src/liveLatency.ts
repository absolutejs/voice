...
await traceStore.append({
  ...
  const rows = report.recent
    .map(
      (sample) =>
        `<tr><td>${escapeHtml(sample.sessionId)}</td><td>${escapeHtml(formatMs(sample.latencyMs))}</td><td>${escapeHtml(sample.status ?? "unknown")}</td><td>${escapeHtml(new Date(sample.at).toLocaleString())}</td></tr>`,
    )
    .join("");
...
export const createVoiceLiveLatencyRoutes = (
  ...
  const htmlPath =
    options.htmlPath === undefined ? "/live-latency" : options.htmlPath;
