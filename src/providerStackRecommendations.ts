...
export type VoiceProviderStackCapabilities<TProvider extends string = string> =
	Partial<
		Record<VoiceProviderStackKind, Partial<Record<TProvider, readonly string[]>>>
	>;
...
const defaultStreamingProviders = new Set([
	'assemblyai',
	'deepgram',
	'openai'
]);
...
const chooseProvider = <TProvider extends string>(
	...
	(priorities.find((provider) =>
		available.includes(provider as TProvider)
	) as TProvider | undefined) ?? available[0];
...
export const createVoiceProviderContractMatrixHTMLHandler =
	...
	async () => {
		...
		const body = await (options.render ?? renderVoiceProviderContractMatrixHTML)(
			report,
			{ title: options.title }
		);
...
>(
	...
	if (htmlPath !== false) {
		routes.get(htmlPath, async () => {
			const response = await createVoiceProviderContractMatrixHTMLHandler(
				options
			)();
...
>(
	...
	const gaps = (['llm', 'stt', 'tts'] as const).map((kind) => {
		const required =
			input.required?.[kind] ?? profileRequiredCapabilities[input.profile][kind];
		...
		const missing = provider
			? required.filter((capability) => !includesCapability(present, capability))
			: [...required];
