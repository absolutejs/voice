...
const demoPayload = Buffer.from(new Uint8Array([1, 2, 3, 4])).toString(
  "base64",
);
...
export const buildVoiceTelephonyMediaReport = (
  input: {
    carriers?: readonly VoiceTelephonyMediaCarrierInput[];
  } = {},
  ...
  const reports = carriers.map((entry) => {
    ...
    const lifecycle = buildMediaTelephonyStreamLifecycleReport({
      carrier: entry.carrier,
      envelopes:
        entry.lifecycleEnvelopes ?? demoLifecycleEnvelopes(entry.carrier),
    });
