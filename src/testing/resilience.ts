...
): Promise<VoiceResilienceScenarioResult> => {
...
export const runVoiceResilienceBenchmark =
  async (): Promise<VoiceResilienceReport> => {
    const scenarios = await Promise.all([
      ...
      runScenario(
        ...
        async ({ adapter, connectNewSocket, disconnect, turns, commit }) => {
          await commit(
            "Duplicate final ids should still produce one turn",
            "same-id",
          );
          await disconnect();
          await connectNewSocket();
      ...
      runScenario(
        ...
        async ({ emitFinal, emitEndOfTurn, turns }) => {
          ...
          if (turns.length !== 1) {
            throw new Error(
              "Repeated end-of-turn events created duplicate turns",
            );
          }
        },
      ),
