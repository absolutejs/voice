...
import { scoreTranscriptAccuracy, type VoiceTranscriptAccuracy } from './accuracy';
...
const calculateTurnPassRate = (turnResults: VoiceSessionBenchmarkTurnResult[]) =>
	turnResults.length > 0 ? countPassedTurns(turnResults) / turnResults.length : 0;

const summarizeScenarioCosts = (turnResults: VoiceSessionBenchmarkTurnResult[]) => {
...
export const runVoiceSessionBenchmarkScenario = async (
	...
	const pushTrace = (
		entry: Omit<VoiceSessionBenchmarkTraceEntry, 'atMs'>
	) => {
		if (!options.trace) {
			return;
		}
	...
	const turnResults: VoiceSessionBenchmarkTurnResult[] = fixture.expectedTurnTexts.map((expectedText, index) => {
...
export const summarizeVoiceSessionBenchmarkSeries = (input: {
	...
	return {
		...
		summary: {
			adapterId: input.adapterId,
			averageElapsedMs: roundMetric(
				average(scenarioAggregates.map((scenario) => scenario.averageElapsedMs)),
				2
			),
			averageFallbackReplayAudioMs: roundMetric(
				average(
					scenarioAggregates.map((scenario) => scenario.averageFallbackReplayAudioMs)
				),
				2
			),
			...
			averageRelativeCostUnits: roundMetric(
				average(
					scenarioAggregates.map((scenario) => scenario.averageRelativeCostUnits)
				)
			),
			averageTurnPassRate: roundMetric(
				average(
...
export const runVoiceSessionBenchmark = async (input: {
	...
	return {
		...
		summary: summarizeVoiceSessionBenchmark(
			input.adapterId,
			scenarioResults
		)
