...
export const applyVoiceAudioCondition = (
  ...
  if (condition.type === "gain") {
    ...
    const chunkSamples = Math.max(
      1,
      Math.round(
        (fixture.format.sampleRateHz * condition.chunkDurationMs) / 1_000,
      ),
    );
    for (
      let start = chunkSamples * (condition.every - 1);
      start < output.length;
      start += chunkSamples * condition.every
    )
      output.fill(0, start, Math.min(output.length, start + chunkSamples));
  } else {
    let state = condition.seed >>> 0;
    const random = () =>
      ((state = (state * 1_664_525 + 1_013_904_223) >>> 0) / 0x1_0000_0000) *
        2 -
      1;
    const signalPower =
      output.reduce((sum, value) => sum + value * value, 0) /
      Math.max(1, output.length);
    const noiseRms = Math.sqrt(signalPower / 10 ** (condition.snrDb / 10));
    for (let index = 0; index < output.length; index += 1)
      output[index] = clamp(
        output[index]! + random() * Math.sqrt(3) * noiseRms,
      );
  }
  return {
    ...fixture,
    audio: bytes(output),
    id: `${fixture.id}--${condition.id}`,
    tags: [...(fixture.tags ?? []), "conditioned", condition.id],
  };
};
...
export const buildVoiceAudioMatrix = (
  ...
  fixtures.flatMap((fixture) =>
    conditions.map((condition) => applyVoiceAudioCondition(fixture, condition)),
  );
