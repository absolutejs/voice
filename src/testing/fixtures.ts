...
const EXTERNAL_FIXTURE_ENV_KEYS = ['VOICE_FIXTURE_DIR', 'VOICE_FIXTURE_DIRS'] as const;
...
const resolveConfiguredFixtureDirectories = async (
	...
	for (const directory of uniqueDirectories) {
		const manifestExists = await Bun.file(resolve(directory, 'manifest.json')).exists();
		if (!manifestExists) {
			throw new Error(
...
const toPcm16Bytes = (samples: Int16Array) =>
	new Uint8Array(samples.buffer.slice(samples.byteOffset, samples.byteOffset + samples.byteLength));
...
export const createMultiSpeakerVoiceTestFixtures = (
	...
	const buildTags = (...tags: string[]) => ['multi-speaker', 'handoff', 'synthetic', ...tags];
