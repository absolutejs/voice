...
import type {
  ...
  VoiceSessionBenchmarkScenarioResult,
} from "./sessionBenchmark";
...
const isUsefulCodeSwitchWindow = (window: CodeSwitchToken[]) => {
  ...
  const scripts = new Set(
    window.map((token) => token.script).filter((script) => script !== "other"),
  );
  ...
  const uniqueAliases = Array.from(
    new Set(
      aliases
        ...
        .filter(
          (alias) =>
            alias.length > 0 && normalizeBenchmarkText(alias) !== normalized,
        ),
    ),
  );
...
const SEEDED_VOCABULARY_MARKERS = BENCHMARK_SEEDED_EXTRA_HINTS.flatMap(
  (hint) => [
...
const usesBenchmarkSeedVocabulary = (
  ...
  const haystacks = [
    ...
    ...(fixture.expectedTerms ?? []).map((term) =>
      normalizeBenchmarkText(term),
    ),
  ];
...
export const applyCorrectedBenchmarkReport = (
  ...
  const correctedFixtures: VoiceSTTBenchmarkFixtureResult[] =
    report.fixtures.map((result) => {
      ...
      const corrected = applyPhraseHintCorrections(
        result.finalText,
        phraseHints,
      );
...
export const applyExperimentalBenchmarkReport = (
  ...
  const correctedFixtures: VoiceSTTBenchmarkFixtureResult[] =
    report.fixtures.map((result) => {
      ...
      const phraseHints = createDomainPhraseHints(
        buildFixtureDomainTerms(fixture),
        {
          riskTier: "balanced",
        },
      );
...
const sliceSTTHoldoutReport = (
  ...
  const fixtures = report.fixtures.filter((fixture) =>
...
export const buildCorrectionBenchmarkAudit = (
  ...
  return {
    ...
    lexicalHoldout: {
      ...
      experimental: sliceSTTHoldoutReport(
        experimental,
...
const summarizeSessionBenchmark = (
  ...
  return {
    ...
    averageWordErrorRate: roundMetric(
      average(
        totalTurnResults.map((turn) => turn.accuracy?.wordErrorRate ?? 0),
      ),
    ),
    duplicateTurnRate:
      scenarioCount > 0
    ...
    reconnectSuccessRate:
      reconnectTriggeredScenarios.length > 0
        ? roundMetric(
            reconnectSuccessCount / reconnectTriggeredScenarios.length,
          )
        : 1,
    scenarioCount,
...
const sliceSessionHoldoutReport = (
  ...
  const scenarios = report.scenarios.filter((scenario) =>
    fixtureIdSet.has(scenario.fixtureId),
  );
...
export const buildSessionCorrectionAudit = (
    ...
    .filter(
      (scenario) =>
        !usesBenchmarkSeedVocabulary({
          ...
          expectedText:
            scenario.expectedTurnTexts?.join(" ") ?? scenario.expectedText,
        }),
    )
    .map((scenario) => scenario.id)
    .filter((fixtureId) =>
      raw.scenarios.some((scenario) => scenario.fixtureId === fixtureId),
    );
  ...
  return {
    ...
    lexicalHoldout: {
      ...
      experimental: sliceSessionHoldoutReport(
        experimental,
        lexicalHoldoutFixtureIds,
      ),
      benchmarkSeeded: sliceSessionHoldoutReport(
