...
export type VoiceSTTBenchmarkAcceptanceThresholds = {
	...
	groupPassRate?: Partial<
		Record<VoiceSTTFixtureEnvironment, { passRate?: number; wordAccuracyRate?: number }>
	>;
};
...
export type VoiceSTTBenchmarkOptions = VoiceSTTAdapterHarnessOptions & {
	...
	openOptions?:
		...
		| ((fixture: VoiceTestFixture) => Partial<STTAdapterOpenOptions> | undefined);
};
...
const scoreSpeakerTurns = (
	...
	const collapsedActualTurns = actualTurns.reduce<typeof actualTurns>((merged, turn) => {
	...
	return {
		...
		expectedTurnCount: expectedTurns.length,
		passes:
			scoredTurns.length === expectedTurns.length &&
			patternMatchRate === 1,
		patternMatchRate,
		postClustered: repaired.postClustered
	};
...
const calculateGroupSummary = (
	...
	const grouped = new Map<VoiceSTTFixtureEnvironment, VoiceSTTBenchmarkFixtureResult[]>();
...
export const summarizeSTTBenchmark = (
	...
	return {
		...
		averageElapsedMs:
			roundMetric(average(fixtures.map((fixture) => fixture.elapsedMs)), 2) ?? 0,
		...
		averageFinalCount:
			roundMetric(average(fixtures.map((fixture) => fixture.finalCount)), 2) ?? 0,
		averageSpeakerTurnMatchRate: roundMetric(
			average(fixtures.map((fixture) => fixture.speakerTurns?.patternMatchRate))
		),
		averageTermRecall:
			roundMetric(
	...
	const fixtureAggregates = [...fixtureMap.entries()].map(([fixtureId, results]) => {
		...
		return {
			averageElapsedMs: roundMetric(
				average(results.map((result) => result.elapsedMs)),
				2
			) ?? 0,
	...
	return {
		...
		summary: {
			...
			averageWordErrorRate: roundMetric(
				average(fixtureAggregates.map((fixture) => fixture.averageWordErrorRate))
			) ?? 0,
			fixtureCount: fixtureAggregates.length,
			flakyFixtureCount: fixtureAggregates.filter(
				(fixture) =>
					fixture.averagePassRate > 0 && fixture.averagePassRate < 1
			).length,
