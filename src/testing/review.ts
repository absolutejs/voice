...
const formatMetric = (
	...
	unit = 'ms'
) => (typeof value === 'number' ? `${label}: ${roundMetric(value)}${unit}` : undefined);
...
const findTimelineEvent = (
	...
	timeline.find(
		(entry) =>
			entry.event === event && (source === undefined || entry.source === source)
	);
...
export const createVoiceCallReviewRecorder = (
	...
	const push = (
		source: ReviewTimelineSource,
		event: string,
		fields: Omit<VoiceCallReviewTimelineEvent, 'atMs' | 'event' | 'source'> = {}
	) => {
		timeline.push({
...
export const renderVoiceCallReviewHTML = (artifact: VoiceCallReviewArtifact) => {
