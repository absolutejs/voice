...
export type VoiceMonitorNotifier = {
  deliver: (
    ...
    | Promise<VoiceMonitorNotifierDeliveryResult>
...
export const deliverVoiceMonitorIssueNotifications = async (
  ...
  const allReceipts = options.receiptStore
    ...
    : receipts;
  const failed = allReceipts.filter(
    (receipt) => receipt.status === "failed",
  ).length;
  const sent = allReceipts.filter(
...
export const createVoiceMonitorRoutes = <TEvidence = unknown>(
  ...
  const htmlPath =
    options.htmlPath === undefined ? "/voice/monitors" : options.htmlPath;
