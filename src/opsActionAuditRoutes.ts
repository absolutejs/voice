...
const readRecord = async (request: Request): Promise<VoiceOpsActionAuditRecord> =>
...
export const createVoiceOpsActionAuditRoutes = (
	...
	if (historyHtmlPath !== false) {
		routes.get(historyHtmlPath, async () =>
			new Response(
...
const toHistoryEntry = (event: StoredVoiceAuditEvent): VoiceOpsActionHistoryEntry => ({
