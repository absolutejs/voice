...
const lifecycleOutcome = (event: StoredVoiceTraceEvent) => {
  ...
  return ["escalation", "no-answer", "transfer", "voicemail"].includes(
    type ?? "",
  )
    ? type
    : undefined;
...
>(
  ...
  const observed = {
    ...
    mediaStarts: events.filter(
      (event) =>
        event.type === "call.lifecycle" && payloadType(event) === "start",
    ).length,
    sessionErrors: events.filter((event) => event.type === "session.error")
      .length,
  ...
  require("carrier-contract", options.contract?.pass ===
    true, "Carrier setup contract is missing or failing.");
  require("media-started", observed.mediaStarts >
    0, "No media-start lifecycle trace was recorded.");
  require("transcript", observed.transcripts >
    0, "No transcript trace was recorded.");
  require("assistant-response", observed.assistantResponses >
    0, "No assistant response trace was recorded.");
  require("lifecycle-outcome", observed.lifecycleOutcomes.length >
    0, "No terminal lifecycle outcome was recorded.");
  require("no-session-error", observed.sessionErrors ===
    0, `${observed.sessionErrors} session error trace(s) were recorded.`);
  require("fresh-trace", typeof options.maxAgeMs !== "number" ||
    (latestEventAt !== undefined &&
      now - latestEventAt <=
        options.maxAgeMs), "Phone-agent smoke trace evidence is stale or missing.");
...
>(
  ...
  ...options,
  contract: (await options.getContract?.(input)) ?? options.contract,
...
>(
  ...
  if (htmlPath) {
    routes.get(
      htmlPath,
      createVoicePhoneAgentProductionSmokeHTMLHandler(options),
    );
  }
