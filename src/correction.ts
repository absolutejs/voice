...
const buildAliasMatcher = (alias: string) =>
	new RegExp(`(?<![\\p{L}\\p{N}'])${escapeRegExp(alias)}(?![\\p{L}\\p{N}'])`, 'giu');
...
const canUseFuzzyAlias = (alias: string) =>
	normalizeComparableText(alias).split(' ').filter((token) => token.length > 0).length >= 2;
...
const lexiconToPhraseHints = (lexicon: VoiceLexiconEntry[]): VoicePhraseHint[] =>
	lexicon.map((entry) => ({
