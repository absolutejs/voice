...
const rebuildRecentJsonFileIndex = async (directory: string) => {
  ...
  const entries = sortRecentJsonFileIndexEntries(
    candidates.filter(
      (candidate): candidate is { path: string; updatedAt: number } =>
        Boolean(candidate),
    ),
  );
...
const removeRecentJsonFileIndexEntry = async (
  directory: string,
  path: string,
) => {
  const files = (await readRecentJsonFileIndex(directory)).filter(
...
const readRecentJsonFiles = async <T>(directory: string, limit: number) => {
  ...
  for (
    let index = 0;
    index < files.length && existingRecords.length < limit;
  ) {
...
const shouldUseRecentReadWindow = (filter: {
  limit?: number;
  readWindow?: string;
...
>(
  ...
  const remove: VoiceAssistantMemoryStore<TRecord>["delete"] = async (
    input,
  ) => {
...
>(
  ...
  const list: VoiceIncidentBundleStore<TArtifact>["list"] = async (
    filter = {},
  ) => {
    const files = await listJsonFiles(options.directory);
