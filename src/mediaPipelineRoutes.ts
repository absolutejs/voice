...
export type VoiceMediaPipelineRoutesOptions =
  VoiceMediaPipelineReportOptions & {
    ...
    source?:
      | (() =>
          | Promise<VoiceMediaPipelineReportOptions>
          | VoiceMediaPipelineReportOptions)
      | VoiceMediaPipelineReportOptions;
    title?: string;
  };
...
export const evaluateVoiceMediaPipelineEvidence = (
  ...
  if ((input.requirePass ?? false) && report.status !== "pass") {
    issues.push(
      `Expected media pipeline proof to pass, found ${report.status}.`,
    );
  }
  if (
  ...
  if (
    ...
    issues.push(
      "Expected resampling plan when calibration requires resampling.",
    );
  }
