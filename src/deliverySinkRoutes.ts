...
export const buildVoiceDeliverySinkReport = async (
	...
	const [auditSummary, traceSummary] = await Promise.all([
		options.auditDeliveries
			? Promise.resolve(
					options.auditDeliveries.store.list()
				).then((deliveries) => summarizeVoiceAuditSinkDeliveries(deliveries))
			: undefined,
		options.traceDeliveries
			? Promise.resolve(
					options.traceDeliveries.store.list()
				).then((deliveries) => summarizeVoiceTraceSinkDeliveries(deliveries))
			: undefined
	]);
	const report = {
...
const renderSurfaceCard = (
	surface: VoiceTraceDeliverySinkSurface<{
		delivered: number;
		total: number;
	}> | undefined
) => {
