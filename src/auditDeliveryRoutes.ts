...
import {
	...
	type VoiceAuditSinkDeliveryQueueStatus,
...
export type VoiceAuditDeliveryDrainWorker = {
	drain: () =>
		| Promise<VoiceAuditSinkDeliveryWorkerResult>
...
export const resolveVoiceAuditDeliveryFilter = (
	query: Record<string, unknown> = {},
	base: VoiceAuditDeliveryFilter = {}
...
const deliverySearchText = (delivery: VoiceAuditSinkDeliveryRecord) =>
	[
		delivery.id,
		...
		...Object.entries(delivery.sinkDeliveries ?? {}).flatMap(([sinkId, result]) => [
			sinkId,
			result.status,
			result.deliveredTo,
			result.error
...
export const buildVoiceAuditDeliveryReport = async (
	...
	const deliveries = filterDeliveries(await options.store.list(), resolvedFilter);
	const summary = await summarizeVoiceAuditSinkDeliveries(deliveries, {
...
const renderMetricGrid = (report: VoiceAuditDeliveryReport) =>
	[
		'<section class="grid">',
...
const renderSinkResults = (delivery: VoiceAuditSinkDeliveryRecord) => {
...
const renderEventList = (delivery: VoiceAuditSinkDeliveryRecord) =>
	delivery.events.length === 0
		? '<p>No audit events in this delivery.</p>'
		: `<ul>${delivery.events
				.map(
					(event) =>
						`<li>${escapeHtml(event.type)} ${escapeHtml(event.action)} <small>${escapeHtml(event.id)}</small></li>`
				)
				.join('')}</ul>`;
...
export const createVoiceAuditDeliveryHTMLHandler =
	...
	async ({ query }: { query?: Record<string, string | undefined> }) => {
		...
		const body = await (options.render?.(report) ??
			renderVoiceAuditDeliveryHTML(report, {
				title: options.title,
...
export const createVoiceAuditDeliveryRoutes = (
	...
	const htmlPath =
		options.htmlPath === undefined ? '/audit/deliveries' : options.htmlPath;
	const workerPath =
	...
	if (htmlPath !== false) {
		routes.get(htmlPath, createVoiceAuditDeliveryHTMLHandler(options));
	}
	...
	if (options.worker && workerPath !== false) {
		routes.post(workerPath, async () => {
			...
			return {
				...result,
				drainedAt: Date.now()
			} satisfies VoiceAuditDeliveryDrainReport;
		});
	}
