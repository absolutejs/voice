...
export type VoiceOperationalStatusRoutesOptions =
...
const deliveryRuntimeStatusToCheck = (
  ...
  const failed = summaries.reduce(
    (total, summary) =>
      total + (summary?.failed ?? 0) + (summary?.deadLettered ?? 0),
    0,
  );
  ...
  if (htmlPath !== false) {
    routes.get(htmlPath, async () => {
      ...
      const body = await (
        options.render ??
        ((input) =>
          renderVoiceOperationalStatusHTML(input, { title: options.title }))
      )(report);
