...
const renderProviderCards = (session: VoiceTraceTimelineSession) =>
	session.providers.length === 0
		...
		: `<div class="providers">${session.providers
				.map(
					(provider) => `<article><strong>${escapeHtml(provider.provider)}</strong><dl><div><dt>Events</dt><dd>${String(provider.eventCount)}</dd></div><div><dt>Avg</dt><dd>${formatMs(provider.averageElapsedMs)}</dd></div><div><dt>Max</dt><dd>${formatMs(provider.maxElapsedMs)}</dd></div><div><dt>Errors</dt><dd>${String(provider.errorCount)}</dd></div><div><dt>Fallbacks</dt><dd>${String(provider.fallbackCount)}</dd></div><div><dt>Timeouts</dt><dd>${String(provider.timeoutCount)}</dd></div></dl></article>`
				)
				.join('')}</div>`;
...
export const createVoiceTraceTimelineRoutes = (
	...
	routes.get(`${path}/:sessionId`, async ({ params }) => {
		...
		return session
			...
			: Response.json({ error: 'Voice trace session not found.' }, { status: 404 });
	});
